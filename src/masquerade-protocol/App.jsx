      ...
      return (
        <div
          ...
          style={{
            ...
            backgroundColor: Math.random() > 0.5 ? "#00c3ff" : "#0095ff",
            animationDuration: `${10 + Math.random() * 20}s`,
            animationDelay: `${Math.random() * -20}s`,
            boxShadow: "0 0 12px 2px rgba(0, 180, 252, 0.8)", // Brighter glow
...
const FloatingBackground = React.memo(() => {
  ...
  const backgroundIcons = React.useMemo(() => {
    return [...Array(20)].map((_, i) => {
      const iconKeys = Object.keys(PACKETS);
      const Icon = PACKETS[iconKeys[i % iconKeys.length]].icon;
      return (
        <div
          key={i}
...
export default function MasqueradeProtocol() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("masqueradeprotocol_roomId") || "";
    }
    return "";
  });
