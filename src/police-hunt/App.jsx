...
import CoverImage from "./assets/police_cover.png";
...
const FloatingBackground = React.memo(() => {
  ...
  const backgroundIcons = React.useMemo(() => {
    return [...Array(20)].map((_, i) => {
      const iconKeys = Object.keys(ROLES);
      const Icon = ROLES[iconKeys[i % iconKeys.length]].icon;
      return (
        <div
          key={i}
      ...
      return (
        <div
          ...
          style={{
            ...
            backgroundColor: Math.random() > 0.5 ? "#e61414" : "#1519e6",
            animationDuration: `${10 + Math.random() * 20}s`,
            animationDelay: `${Math.random() * -20}s`,
            boxShadow: "0 0 12px 2px rgba(12, 132, 237, 0.8)", // Brighter glow
...
const SplashScreen = ({ onStart }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col items-center justify-end pb-20 md:justify-center md:pb-0 font-sans overflow-hidden">
      ...
      {!isLoaded && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-50 text-blue-500/50">
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
        ...
        >
          <button
            onClick={onStart}
            className="group relative px-12 py-5 bg-blue-600/20 hover:bg-blue-600/40 border border-blue-500/50 hover:border-blue-400 text-blue-300 font-black text-2xl tracking-widest rounded-none transform transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(34,211,238,0.4)] backdrop-blur-md overflow-hidden"
          >
            {/* Animated Scanline overlay */}
            <div className="absolute inset-0 bg-gradient-to-b from-transparent via-blue-400/10 to-transparent translate-y-[-100%] animate-[scan_2s_infinite_linear]" />
...
export default function ThiefPoliceGame() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("policehunt_roomId") || "";
    }
    return "";
  });
  ...
  if (!user)
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center text-blue-500 animate-pulse">
        Siren wailing...
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
        ...
        <nav className="absolute top-0 left-0 w-full p-4 z-50">
          <a
            ...
            className="flex items-center gap-2 text-blue-800 rounded-lg 
			font-bold shadow-md hover:text-blue-400 transition-colors w-fit animate-pulse"
          >
            {/* Arrow Icon */}
            <StepBack />
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <div className="relative">
                  ...
                  {/* 3. The Copied Popup */}
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded shadow-lg animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
      ...
      >
        ...
        <div className="bg-slate-800/90 backdrop-blur-md p-3 flex justify-between items-center shadow-lg z-10 border-b border-slate-700">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => setShowLeaveConfirm(true)}
              className="bg-slate-700 p-2 rounded hover:bg-red-900/50 hover:text-red-400 transition-colors text-red-500"
              title="Leave Game"
            >
              <LogOut size={20} />
            </button>
