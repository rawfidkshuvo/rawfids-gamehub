...
import CoverImage from "./assets/royal_cover.png";
...
const FloatingBackground = React.memo(() => {
  ...
  const backgroundIcons = React.useMemo(() => {
    return [...Array(20)].map((_, i) => {
      const iconKeys = Object.keys(ANIMALS);
      const Icon = ANIMALS[iconKeys[i % iconKeys.length]].icon;
      return (
        <div
          key={i}
...
export default function RoyalMenagerie() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("royal_menagerie_roomId") || "";
    }
    return "";
  });
