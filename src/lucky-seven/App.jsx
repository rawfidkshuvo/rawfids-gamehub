...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "lucky-seven";
const GAME_ID = "24";
      ...
      return (
        <div
          ...
          style={{
            ...
            backgroundColor: Math.random() > 0.5 ? "#009dff" : "#332ddc",
            animationDuration: `${10 + Math.random() * 20}s`,
            animationDelay: `${Math.random() * -20}s`,
            boxShadow: "0 0 12px 2px rgba(0, 168, 247, 0.8)", // Brighter glow
...
const GENERATE_DECK = () => {
  ...
  [2, 4, 6, 8, 10].forEach((val) =>
    deck.push({ id: `c_${idCounter++}`, type: "MODIFIER", value: `+${val}` }),
  );
...
const GameCard = ({ card, size = "md", isBust = false }) => {
  const sizeClasses =
    size === "sm"
      ? "w-14 h-20 sm:w-16 sm:h-24 border sm:border-2 shrink-0"
      : size === "lg"
        ? "w-24 h-36 sm:w-32 sm:h-48 border-2 sm:border-4 shrink-0"
  ...
  if (card.type === "NUMBER") {
    bgClass =
      "bg-gradient-to-br from-cyan-500 to-blue-800 shadow-[0_5px_15px_rgba(6,182,212,0.4)]";
    borderClass = "border-cyan-300/80";
    textClass = "text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]";
  } else if (card.type === "MODIFIER") {
    bgClass =
      "bg-gradient-to-br from-amber-400 to-orange-700 shadow-[0_5px_15px_rgba(245,158,11,0.4)]";
    borderClass = "border-amber-200/80";
    textClass = "text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]";
  } else if (card.type === "ACTION") {
    bgClass =
      "bg-gradient-to-br from-rose-500 to-fuchsia-900 shadow-[0_5px_15px_rgba(244,63,94,0.4)]";
    borderClass = "border-rose-300/80";
    textClass = "text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]";
    if (card.value === "FREEZE")
      icon = (
        <Snowflake className="w-2/3 h-2/3 opacity-20 absolute text-white" />
      );
    if (card.value === "FLIP3")
      icon = <Shuffle className="w-2/3 h-2/3 opacity-20 absolute text-white" />;
    if (card.value === "SECOND_CHANCE")
      icon = (
        <HeartHandshake className="w-2/3 h-2/3 opacity-20 absolute text-white" />
      );
  }

  return (
    <div
      className={`${sizeClasses} relative rounded-xl flex flex-col items-center justify-center ${bgClass} ${borderClass} transition-all duration-300 overflow-hidden ${isBust ? "ring-4 ring-red-500 animate-pulse grayscale" : "hover:-translate-y-1 hover:shadow-[0_15px_25px_rgba(255,255,255,0.2)]"}`}
    >
      ...
      <span
        className={`relative z-10 font-black tracking-tight ${size === "lg" ? "text-5xl sm:text-6xl" : size === "sm" ? "text-xl sm:text-2xl" : "text-3xl sm:text-4xl"} ${textClass}`}
      >
        {card.type === "ACTION"
          ? card.value === "SECOND_CHANCE"
            ? "2ND"
            : card.value.replace("FREEZE", "FRZ")
          : card.value}
      </span>

      {card.type === "ACTION" && size !== "sm" && (
        <span
          className={`relative z-10 text-[8px] sm:text-[10px] font-black tracking-widest uppercase mt-1 sm:mt-2 text-center leading-tight px-1 ${textClass}`}
        >
...
const FeedbackOverlay = ({ type, message, subtext, icon: Icon }) => (
  <div className="fixed inset-0 z-[300] flex items-center justify-center pointer-events-none animate-in fade-in zoom-in duration-300">
    <div
      className={`flex flex-col items-center justify-center p-6 sm:p-12 rounded-3xl border-4 shadow-2xl backdrop-blur-xl max-w-[90vw] sm:max-w-xl mx-auto text-center ${
        type === "success"
    ...
    >
      {Icon && (
        <div className="mb-4 p-3 sm:p-4 bg-black/20 rounded-full">
          <Icon className="w-10 h-10 sm:w-16 sm:h-16 animate-bounce" />
        </div>
      )}
      <h2 className="text-2xl sm:text-5xl font-black uppercase tracking-widest drop-shadow-md mb-2 leading-tight">
        {message}
      </h2>
      {subtext && (
        <p className="text-sm sm:text-xl font-bold opacity-90 tracking-wide">
          {subtext}
        </p>
      )}
    </div>
...
const RulesModal = ({ onClose }) => (
  <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-4">
    <div className="bg-slate-900 border border-cyan-900/50 w-full max-w-3xl rounded-3xl shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto custom-scrollbar flex flex-col">
      ...
      <h2 className="text-2xl sm:text-3xl font-black text-center mb-6 text-cyan-400">
        How to Play Lucky 7
      </h2>
      <div className="space-y-4 sm:space-y-6 text-xs sm:text-sm text-slate-300 flex-1">
        ...
        <section>
          <h3 className="text-lg sm:text-xl font-bold text-white mb-2 flex items-center gap-2">
            <Hash className="text-cyan-400 w-5 h-5" /> Busting & Lucky 7
          </h3>
          <p className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-cyan-900/30 mb-2">
            You bust if you flip a Number card that you{" "}
            <strong>already have</strong> in front of you. Busted players score
            0 for the turn. (Note: Modifiers and Actions do not cause busts).
          </p>
          <p className="bg-cyan-900/20 p-3 sm:p-4 rounded-xl border border-cyan-500/50 text-cyan-100 font-bold">
            If you successfully reveal 7 unique Number cards, you instantly end
            the round and score a +15 point bonus!
          </p>
        </section>
        <section>
          <h3 className="text-lg sm:text-xl font-bold text-white mb-2 flex items-center gap-2">
            <Zap className="text-cyan-400 w-5 h-5" /> Special Cards
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-slate-800 p-3 rounded-xl border border-amber-500/30">
              ...
              <p className="text-[10px] sm:text-xs">
                Boost your score when you bank.
              </p>
            </div>
            <div className="bg-slate-800 p-3 rounded-xl border border-rose-500/30">
              ...
              <p className="text-[10px] sm:text-xs">
                Target a player. Their turn immediately ends and their points
                are banked.
              </p>
            </div>
            <div className="bg-slate-800 p-3 rounded-xl border border-rose-500/30">
              ...
              <p className="text-[10px] sm:text-xs">
                Target a player. They are forced to flip 3 more cards on their
                turn.
              </p>
            </div>
            <div className="bg-slate-800 p-3 rounded-xl border border-rose-500/30">
              ...
              <p className="text-[10px] sm:text-xs">
                If you bust, automatically discard this and the duplicate card
                to survive.
              </p>
            </div>
          </div>
      ...
      <div className="mt-6 sm:mt-8 flex flex-col items-center gap-2 shrink-0">
        <button
          onClick={onClose}
          className="px-6 py-2 sm:px-8 sm:py-3 rounded-xl font-bold text-base sm:text-lg shadow-lg transition-all bg-gradient-to-br from-cyan-600 to-blue-800 text-white hover:from-cyan-500 hover:to-blue-700 hover:scale-105"
        >
          Return to Game
        </button>
      </div>
...
const ScoreboardModal = ({ gameState, onClose }) => (
  <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex items-center justify-center pt-16 sm:pt-20 pb-10 px-4">
    <div className="bg-slate-900 border border-cyan-900/50 w-full max-w-lg rounded-3xl shadow-2xl p-4 sm:p-6 relative flex flex-col max-h-full">
      <button
        onClick={onClose}
        className="absolute top-3 sm:top-4 right-3 sm:right-4 p-2 bg-slate-800 rounded-full hover:bg-slate-700 transition-colors z-10"
      >
        <X className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
      </button>
      <div className="shrink-0 mb-4 sm:mb-6">
        <h2 className="text-xl sm:text-2xl font-black text-center text-cyan-400 flex items-center justify-center gap-2">
          <BarChart2 className="w-5 h-5 sm:w-6 sm:h-6" /> Scoreboard
        </h2>
      </div>
        ...
        {gameState.players
          ...
          .map((p, idx) => {
            return (
              <div
                key={p.id}
                className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-slate-700 flex items-center justify-between"
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  <span className="text-lg sm:text-2xl font-black text-slate-500 w-5 sm:w-6">
                    #{idx + 1}
                  </span>
                  <div
                    className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full shadow-lg ${PLAYER_COLORS[p.colorIdx].bg}`}
                  />
                  <span className="font-bold text-sm sm:text-lg text-white truncate max-w-[120px] sm:max-w-[200px]">
                    {p.name}
                  </span>
                </div>
                <span className="text-xl sm:text-3xl font-black text-amber-400">
                  {p.score}{" "}
...
export default function Lucky7Game() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("luckyseven_roomId") || "";
    }
    return "";
  });
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          if (!data.players.some((p) => p.id === user.uid)) {
            setRoomId("");
            localStorage.removeItem("luckyseven_roomId");
            setView("menu");
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (snap.exists() && snap.data().status === "lobby") {
        ...
        setRoomId(code);
        localStorage.setItem("luckyseven_roomId", code);
  ...
  const startGame = async () => {
    ...
    let players = gameState.players.map((p) => ({
      ...p,
      score: 0,
      activeCards: [],
      status: "PLAYING",
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        logs: arrayUnion(
          triggerLog(
            "Cards dealt. Let the flipping begin!",
            "neutral",
            true,
            "START",
          ),
  ...
  const kickPlayer = async (targetId) => {
    ...
    try {
      ...
      await updateDoc(
        ...
        {
          players: newPlayers,
          logs: arrayUnion(triggerLog("A player was removed.", "warning")),
        },
      );
    } catch (e) {
  ...
  const flipCard = async () => {
    ...
    if (gameState.turnIndex !== pIdx || gameState.turnPhase !== "PLAYING")
      return;
    ...
    if (card.type === "NUMBER") {
      const hasDuplicate = me.activeCards.some(
        (c) => c.type === "NUMBER" && c.value === card.value,
      );
      if (hasDuplicate) {
        const scIdx = me.activeCards.findIndex(
          (c) => c.value === "SECOND_CHANCE",
        );
        if (scIdx !== -1) {
          ...
          logs.push(
            triggerLog(
              `${me.name} used a Second Chance to survive a ${card.value}!`,
              "success",
            ),
          );
          ...
          logs.push(
            triggerLog(
              `${me.name} busted on a ${card.value}!`,
              "failure",
              true,
              "BUSTED!",
            ),
        ...
        const uniques = new Set(
          me.activeCards.filter((c) => c.type === "NUMBER").map((c) => c.value),
        );
        if (uniques.size >= 7) {
          me.status = "BANKED";
      ...
      if (card.value === "SECOND_CHANCE") {
        const anyoneNeedsIt = players.some(
          (p) =>
            p.status === "PLAYING" &&
            !p.activeCards.some((c) => c.value === "SECOND_CHANCE"),
        );
        ...
        if (!anyoneNeedsIt) {
          ...
          if (me.forcedFlips > 0) me.forcedFlips--;
          logs.push(
            triggerLog("Everyone has a Second Chance. Discarded.", "neutral"),
          );
          ...
          logs.push(
            triggerLog(
              `${me.name} must pass the Second Chance to another player.`,
              "warning",
            ),
          );
        }
        ...
        logs.push(
          triggerLog(
            `${me.name} must target a player with ${card.value.replace("_", " ")}.`,
            "warning",
          ),
        );
      }
      ...
      if (me.forcedFlips > 0) me.forcedFlips--;
      logs.push(
        triggerLog(`${me.name} flipped a ${card.value} modifier.`, "neutral"),
      );
    }
  ...
  const resolveAction = async (targetId) => {
    const pIdx = gameState.players.findIndex((p) => p.id === user.uid);
    if (gameState.turnPhase !== "ACTION_TARGET" || gameState.turnIndex !== pIdx)
      return;
    ...
    if (target.status !== "PLAYING")
      return alert("You can only target players who are still playing.");
    ...
    if (actionCard.value === "FREEZE") {
      ...
      logs.push(
        triggerLog(`${me.name} forced ${target.name} to Flip 3!`, "warning"),
      );
      discardPile.push(actionCard);
    } else if (actionCard.value === "SECOND_CHANCE") {
      if (target.activeCards.some((c) => c.value === "SECOND_CHANCE"))
        return alert("That player already has a Second Chance!");
      target.activeCards.push(actionCard);
      logs.push(
        triggerLog(
          `${me.name} gave ${target.name} a Second Chance!`,
          "success",
        ),
      );
    }
  ...
  const bankScore = async () => {
    ...
    if (
      ...
      gameState.players[pIdx].forcedFlips > 0
    )
      return;
    ...
    updates.logs = arrayUnion(
      triggerLog(`${players[pIdx].name} decided to bank.`, "neutral"),
    );
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      updates,
  ...
  const hostScoreRound = async () => {
    if (gameState.hostId !== user.uid || gameState.turnPhase !== "ROUND_END")
      return;

    let players = JSON.parse(JSON.stringify(gameState.players));
    let discardPile = [...gameState.discardPile];
    let deck = [...gameState.deck];
    let logs = [triggerLog("Scoring complete.", "neutral")];

    // 1. Calculate Scores & Discard active cards
    players.forEach((p) => {
      if (p.status === "BANKED") {
        let sum = 0;
        let multi = 1;
        const numbers = (p.activeCards || [])
    ...
    // 2. Check for Winner or Sudden Death Tiebreaker
    ...
    if (highestScore >= 200) {
      const topScorers = activePlayers.filter((p) => p.score === highestScore);

      if (topScorers.length === 1) {
        isGameFinished = true; // Clear winner
      } else {
        // Tiebreaker!
        logs.push(
          triggerLog(
            "SUDDEN DEATH TIEBREAKER! Top scorers battle to break the tie!",
            "important",
            true,
            "SUDDEN DEATH!",
          ),
        ...
        // Eliminate anyone who isn't tied for the lead
        players.forEach((p) => {
          if (p.score < highestScore && p.status !== "ELIMINATED") {
            p.status = "ELIMINATED";
          }
        });
      }
    ...
    // 3. Prepare the next round or end game
    let updates = {};
    ...
    if (isGameFinished) {
      ...
      // Reset surviving players for the next round
      players.forEach((p) => {
      ...
      // Deal 1 starting card only to surviving players
      const survivingPlayers = players.filter((p) => p.status !== "ELIMINATED");
      if (deck.length < survivingPlayers.length) {
      ...
      // Determine next turn index (skip eliminated players)
      let nextTurn = (gameState.turnIndex + 1) % players.length;
      let loops = 0;
      while (
        players[nextTurn].status === "ELIMINATED" &&
        ...
        <a href={import.meta.env.BASE_URL}>
          <div className="flex items-center justify-center gap-3 mb-2">
            <div className="text-center pb-12 animate-pulse">
              <div className="inline-flex items-center gap-2 sm:gap-3 px-6 py-3 sm:px-8 sm:py-4 bg-slate-900/50 rounded-full border border-cyan-900/20 text-cyan-300 font-bold tracking-widest text-xs sm:text-sm uppercase backdrop-blur-sm">
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans select-none">
        ...
        <nav className="absolute top-0 left-0 w-full p-4 z-50">
          <a
            ...
            className="flex items-center gap-2 text-cyan-800 rounded-lg font-bold shadow-md hover:text-cyan-400 transition-colors w-fit animate-pulse text-sm sm:text-base"
          >
            <StepBack className="w-4 h-4 sm:w-6 sm:h-6" />
        ...
        <div className="z-10 text-center mb-8 sm:mb-10 mt-8">
          ...
          <p className="text-cyan-200/40 tracking-[0.3em] sm:tracking-[0.5em] uppercase mt-2 text-[10px] sm:text-xs font-bold">
            Flip. Risk. Bank.
          </p>
        </div>
        <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-6 sm:p-8 rounded-3xl w-full max-w-md shadow-2xl z-10 relative">
          {error && (
            <div className="bg-red-900/20 border border-red-500/50 text-red-200 p-3 mb-4 rounded text-center text-xs sm:text-sm font-bold flex items-center justify-center gap-2">
              <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5" /> {error}
            </div>
          )}
          <div className="space-y-4">
            <input
              className="w-full bg-black/50 border border-cyan-900/50 focus:border-cyan-400 p-3 sm:p-4 rounded-xl text-white outline-none transition-all text-base sm:text-lg font-bold text-center shadow-inner"
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                onClick={createRoom}
                disabled={loading}
                className="bg-gradient-to-br from-cyan-500 to-blue-700 hover:from-cyan-400 hover:to-blue-600 p-3 sm:p-4 rounded-xl font-bold flex flex-col items-center justify-center gap-1 transition-all active:scale-95 shadow-[0_0_20px_rgba(6,182,212,0.4)] border border-cyan-300/30"
              >
              ...
              <div className="flex flex-col gap-2">
                <input
                  className="bg-black/50 border border-cyan-900/50 focus:border-cyan-400 p-2 rounded-xl text-white text-center uppercase font-mono font-bold tracking-widest outline-none h-10 sm:h-12 text-sm sm:text-base shadow-inner"
                  placeholder="CODE"
                  value={roomCode}
            ...
            <button
              ...
              className="w-full mt-4 text-cyan-500 hover:text-cyan-400 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-colors py-2"
            >
              <BookOpen className="w-4 h-4 sm:w-5 sm:h-5" /> How to Play
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 sm:p-6 relative">
        ...
        <div className="z-10 w-full max-w-lg bg-slate-900/90 backdrop-blur-xl p-6 sm:p-8 rounded-3xl border border-white/10 shadow-2xl animate-in slide-in-from-bottom-8 mt-6">
          <div className="flex justify-between items-center mb-6 sm:mb-8 border-b border-white/10 pb-4">
            <div>
              <h2 className="text-base sm:text-xl flex items-center gap-2 text-cyan-400 font-black uppercase tracking-widest">
                <Earth className="w-5 h-5 sm:w-6 sm:h-6" /> Table Code
              </h2>
              <div className="flex items-center gap-2 sm:gap-3 mt-1">
                <div className="text-2xl sm:text-4xl font-mono text-white font-black drop-shadow-md">
                  {roomId}
                </div>
                <div className="relative">
                  ...
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-cyan-500 text-white text-[10px] sm:text-xs font-bold px-2 py-1 rounded shadow-[0_0_10px_rgba(6,182,212,0.5)] animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-2 bg-white/5 hover:bg-rose-900/50 rounded-full text-rose-400 transition-colors border border-white/10"
              >
          ...
          <div className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
            <h3 className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2">
              <User size={14} /> Players ({gameState.players.length}/6)
            </h3>
            {gameState.players.map((p) => (
              <div
                key={p.id}
                className="flex justify-between items-center bg-black/30 p-3 sm:p-4 rounded-xl border border-white/5 shadow-inner"
              >
                <span className="font-bold flex items-center gap-2 sm:gap-3 text-sm sm:text-lg">
                  <div
                    className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full ${PLAYER_COLORS[p.colorIdx].bg} ${PLAYER_COLORS[p.colorIdx].glow}`}
                  />{" "}
                  {p.name}{" "}
                  {p.id === gameState.hostId && (
                    <Crown className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400 drop-shadow-[0_0_5px_rgba(250,204,21,0.6)]" />
                  )}
                </span>
                {gameState.hostId === user.uid && p.id !== user.uid && (
                  <button
                    onClick={() => kickPlayer(p.id)}
                    className="p-1.5 sm:p-2 bg-rose-900/20 hover:bg-rose-600 text-rose-400 hover:text-white rounded-lg transition-colors border border-rose-900/50"
                    title="Kick Player"
                  >
            ...
            {Array.from({ length: 6 - gameState.players.length }).map(
              (_, i) => (
                <div
                  key={i}
          ...
          {isHost ? (
            <div className="flex flex-col gap-2 mt-4">
              <button
                onClick={startGame}
                disabled={!canStart}
                className="w-full flex justify-center items-center gap-2 px-6 py-3 sm:px-8 sm:py-4 rounded-xl font-black tracking-widest text-base sm:text-lg transition-all bg-gradient-to-br from-cyan-500 to-blue-700 text-white hover:from-cyan-400 hover:to-blue-600 hover:scale-[1.02] shadow-[0_0_25px_rgba(6,182,212,0.4)] disabled:opacity-50 disabled:hover:scale-100 border border-cyan-300/30"
              >
                <Zap className="w-5 h-5 sm:w-6 sm:h-6" /> Deal Cards
              </button>
              {!canStart && (
                <div className="text-center text-[10px] sm:text-xs font-bold text-cyan-500 uppercase tracking-wider mt-1">
                  Requires 2 to 6 players to start
                </div>
              )}
            </div>
          ) : (
            <div className="text-center text-slate-400 text-xs sm:text-sm font-bold uppercase tracking-widest animate-pulse border border-white/5 bg-black/30 py-4 rounded-xl">
              Waiting for host...
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-slate-900 border border-white/10 p-6 rounded-2xl max-w-xs w-full text-center shadow-2xl">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={handleLeave}
                  className="flex-1 bg-rose-600 hover:bg-rose-500 py-3 rounded-xl font-bold text-white text-sm sm:text-base transition-colors shadow-[0_0_15px_rgba(225,29,72,0.4)]"
                >
                  Leave
                </button>
              </div>
  ...
  if (view === "game" && gameState) {
    ...
    return (
      <div className="fixed inset-0 bg-slate-950 text-white flex flex-col overflow-hidden font-sans select-none">
        ...
        <div className="h-14 sm:h-16 bg-slate-900/80 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-2 z-[160] shrink-0 shadow-lg relative">
          <div className="flex items-center gap-3 sm:gap-4 relative z-10">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-cyan-500/20 rounded-lg flex items-center justify-center border border-cyan-400/50 ml-1 sm:ml-2 shadow-[0_0_15px_rgba(34,211,238,0.3)]">
              <Zap className="text-cyan-400 w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <div>
              <div className="font-black text-xs sm:text-sm tracking-widest text-white drop-shadow-md">
                LUCKY 7
              </div>
              <div className="text-[8px] sm:text-[10px] font-bold uppercase tracking-wider">
                {gameState.status === "finished" ? (
                  ...
                  <>
                    ...
                    <span className="text-cyan-400">{activePlayer?.name}</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2 relative z-10">
            <button
              ...
              className="p-1.5 sm:p-2 hover:bg-white/10 rounded-full text-amber-400 hover:text-white transition-colors"
            >
              <BarChart2 className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={() => setShowGuide(true)}
              className="p-1.5 sm:p-2 hover:bg-white/10 rounded-full text-slate-300 hover:text-white transition-colors"
            >
              <BookOpen className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={() => setShowLogs(!showLogs)}
              className={`p-1.5 sm:p-2 rounded-full transition-colors ${showLogs ? "bg-cyan-500/20 text-cyan-400" : "text-slate-300 hover:bg-white/10"}`}
            >
              <History className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={() => setShowLeaveConfirm(true)}
              className="p-1.5 sm:p-2 hover:bg-rose-900/50 rounded-full text-rose-400 transition-colors ml-1"
            >
        ...
        {showLogs && (
          <div className="fixed top-14 sm:top-16 right-2 sm:right-4 w-56 sm:w-64 max-h-56 sm:max-h-60 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl z-[155] overflow-y-auto p-2 shadow-2xl custom-scrollbar">
            <h4 className="text-[10px] sm:text-xs font-black text-white uppercase tracking-widest mb-2 sticky top-0 bg-slate-900/95 py-1 sm:py-2 border-b border-white/5">
              World History
            </h4>
        ...
        <div className="w-full bg-black/40 border-b border-white/5 flex overflow-x-auto custom-scrollbar py-2 px-2 gap-2 shadow-inner">
          {gameState.players.map((p) => (
            <div
              key={p.id}
              className={`flex items-center gap-2 px-3 py-1 rounded-full border ${p.id === activePlayer?.id ? "bg-cyan-900/50 border-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.3)]" : "bg-slate-900 border-white/10"} shrink-0 transition-all`}
            >
              <div
                className={`w-2 h-2 rounded-full ${PLAYER_COLORS[p.colorIdx].bg} ${PLAYER_COLORS[p.colorIdx].glow}`}
              />
            ...
            {gameState.players.map((p, idx) => {
              ...
              return (
                <div
                  key={p.id}
                  onClick={() => {
                    if (isTargetable) resolveAction(p.id);
                  }}
      ...
      ${p.status === "ELIMINATED" ? "opacity-20 grayscale" : !isActive ? "opacity-50 grayscale hover:grayscale-0 transition-all" : ""}
   `}
                >
                  {/* Glass Header */}
                  <div
                    className={`absolute top-0 left-0 right-0 h-14 ${color.matBg} backdrop-blur-md border-b border-white/5`}
                  ></div>
                  ...
                  <div className="flex justify-between items-center mb-6 relative z-10">
                    <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-full border border-white/10 shadow-inner">
                      <div
                        className={`w-3 h-3 rounded-full ${color.bg} ${color.glow}`}
                      />
                      <span
                        className={`font-black uppercase tracking-widest text-xs sm:text-sm drop-shadow-md ${color.text}`}
                      >
                        {p.name}
                      </span>
                    </div>
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 pointer-events-none">
                    {p.status === "BUSTED" && (
                      <div className="bg-rose-900/90 backdrop-blur-sm text-rose-200 border-2 border-rose-500 px-6 py-2 rounded-2xl font-black text-3xl uppercase transform -rotate-12 shadow-[0_0_30px_rgba(225,29,72,0.6)]">
                        Busted
                      </div>
                    )}
                    {p.status === "BANKED" && (
                      <div className="bg-amber-900/90 backdrop-blur-sm text-amber-200 border-2 border-amber-500 px-6 py-2 rounded-2xl font-black text-3xl uppercase transform -rotate-12 shadow-[0_0_30px_rgba(245,158,11,0.6)]">
                        Banked
                      </div>
                    )}
                    {p.status === "ELIMINATED" && (
                      <div className="bg-slate-900/90 backdrop-blur-sm text-slate-400 border-2 border-slate-600 px-6 py-2 rounded-2xl font-black text-xl sm:text-2xl uppercase transform -rotate-12 shadow-[0_0_30px_rgba(0,0,0,0.8)]">
                        Eliminated
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 sm:gap-3 overflow-x-auto custom-scrollbar pb-2 items-center relative z-10 min-h-[100px]">
                    ...
                    {(!p.activeCards || p.activeCards.length === 0) &&
                      isActive && (
                        <div className="w-14 h-20 sm:w-16 sm:h-24 border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center text-slate-500 text-[10px] font-black uppercase tracking-widest bg-black/20">
                          Empty
                        </div>
                      )}
                  </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/10 flex flex-col z-[60] shadow-[0_-20px_40px_rgba(0,0,0,0.5)]">
            <div className="flex justify-between items-center px-4 py-2 border-b border-white/5 bg-black/30 shrink-0">
              <div className="flex items-center gap-2">
                <span className="text-[10px] sm:text-xs font-black text-amber-400 bg-amber-500/10 border border-amber-500/30 px-3 py-0.5 rounded-full uppercase drop-shadow-md">
                  Score: {me?.score || 0}
                </span>
              </div>
                ...
                <>
                  {gameState.turnPhase === "PLAYING" ? (
                    <div className="flex gap-4 w-full">
                      <button
                        onClick={flipCard}
                        className="flex-1 bg-gradient-to-br from-cyan-500 to-blue-700 hover:from-cyan-400 hover:to-blue-600 border border-cyan-300/50 text-white font-black py-4 rounded-2xl uppercase tracking-widest shadow-[0_0_25px_rgba(6,182,212,0.4)] transition-all animate-pulse-cyan text-sm sm:text-xl"
                      >
                        FLIP CARD
                      </button>
                      <button
                    ...
                    <div className="flex flex-row items-center justify-center gap-3 w-full p-2 sm:p-4 border border-rose-400/50 rounded-2xl bg-gradient-to-r from-rose-900/30 via-fuchsia-900/30 to-rose-900/30 shadow-[0_0_30px_rgba(244,63,94,0.3)]">
                      <div className="shrink-0 animate-bounce">
                        <GameCard
                          card={gameState.pendingAction?.card}
                          size="sm"
                        />
                      </div>
                      <div className="flex flex-col text-left">
                        <span className="text-rose-400 text-sm sm:text-2xl font-black uppercase tracking-widest leading-none drop-shadow-md">
                          Action Required
                        </span>
                        <span className="text-white text-[10px] sm:text-sm font-bold uppercase mt-1 opacity-90 tracking-wide leading-tight">
                          Target a player above to use
                          <br className="sm:hidden" />{" "}
                          {gameState.pendingAction?.card?.value?.replace(
                            "_",
                            " ",
                          )}
                        </span>
                    ...
                    <div className="w-full text-center text-slate-500 text-xs font-bold uppercase tracking-widest py-5 border border-white/5 rounded-2xl bg-black/20">
                      Please Wait...
                    </div>
                  )}
                </>
                ...
                <>
                  {gameState.turnPhase === "ACTION_TARGET" ? (
                    <div className="w-full flex flex-row items-center justify-center gap-3 py-2 sm:py-4 border border-white/10 rounded-2xl bg-black/40 shadow-inner">
                      ...
                      <div className="flex flex-col text-left">
                        <span className="text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest leading-tight">
                          Waiting for {activePlayer?.name}
                        </span>
                        <span className="text-rose-400 text-xs sm:text-sm font-black uppercase drop-shadow-md leading-tight">
                          To use{" "}
                          {gameState.pendingAction?.card?.value?.replace(
                            "_",
                            " ",
            ...
            {gameState.turnPhase === "ROUND_END" &&
              gameState.hostId === user.uid && (
                <div className="px-4 pb-4 w-full max-w-2xl mx-auto">
                  <button
                    onClick={hostScoreRound}
                    className="w-full bg-gradient-to-br from-emerald-500 to-teal-700 hover:from-emerald-400 hover:to-teal-600 border border-emerald-300/50 text-white font-black py-4 rounded-2xl uppercase tracking-widest shadow-[0_0_25px_rgba(16,185,129,0.4)] transition-all animate-bounce text-sm sm:text-lg"
                  >
                    Score Round & Deal
                  </button>
                </div>
          ...
          <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center pt-16 sm:pt-20 pb-10 px-4">
            <div className="bg-slate-900 p-6 md:p-8 rounded-3xl border border-cyan-500/50 text-center shadow-[0_0_80px_rgba(6,182,212,0.3)] animate-in zoom-in max-w-lg w-full flex flex-col relative max-h-[90vh]">
              <div className="shrink-0 mb-4 sm:mb-6">
                ...
                <p className="text-cyan-400 font-black tracking-[0.3em] text-xs sm:text-sm uppercase drop-shadow-md">
                  Ultimate Flipper
                </p>
              </div>
              ...
              <div className="space-y-3 mb-6 sm:mb-8 overflow-y-auto custom-scrollbar flex-1 px-1">
                {gameState.players
                  ...
                  .map((p, i) => (
                    <div
                      key={p.id}
                      className={`flex justify-between items-center px-4 sm:px-6 py-3 rounded-2xl border ${i === 0 ? "bg-gradient-to-r from-amber-900/40 to-orange-900/40 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)]" : "bg-black/40 border-white/5 shadow-inner"}`}
                    >
                      <span className="font-black text-white text-base sm:text-xl tracking-wide truncate mr-2 drop-shadow-md">
                        {i + 1}. {p.name}
                      </span>
                      <span className="font-black text-yellow-400 text-lg sm:text-2xl shrink-0 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]">
                        {p.score} pt
                      </span>
                    </div>
                  ))}
              ...
              {gameState.hostId === user.uid ? (
                <div className="shrink-0 pt-2">
                  <button
                    ...
                    className="bg-gradient-to-br from-cyan-500 to-blue-700 hover:from-cyan-400 hover:to-blue-600 border border-cyan-300/50 px-4 sm:px-6 py-4 rounded-2xl font-black w-full text-white transition-all uppercase tracking-widest shadow-[0_0_20px_rgba(6,182,212,0.4)] text-sm sm:text-base"
                  >
                    Play Again
                  </button>
                </div>
                ...
                <div className="shrink-0 pt-2">
                  <button
                    disabled
                    className="bg-slate-800 border border-white/5 px-4 sm:px-6 py-4 rounded-2xl font-black w-full text-slate-500 uppercase tracking-widest text-sm sm:text-base shadow-inner"
                  >
                    Waiting for Host...
                  </button>
                </div>
