...
import CoverImage from "./assets/spectrum_cover.png";
...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "spectrum";
const GAME_ID = "16";
...
const FloatingBackground = React.memo(() => {
  ...
  const backgroundIcons = React.useMemo(() => {
    return [...Array(20)].map((_, i) => {
      const iconKeys = Object.keys(SUITS);
      const Icon = SUITS[iconKeys[i % iconKeys.length]].icon;
      return (
        <div
          key={i}
...
const SplashScreen = ({ onStart }) => {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("spectrum_roomId");
    setHasSession(!!saved);
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans">
        ...
        <nav className="absolute top-0 left-0 w-full p-4 z-50">
          <a
            ...
            className="flex items-center gap-2 text-fuchsia-800 rounded-lg 
			font-bold shadow-md hover:text-fuchsia-400 transition-colors w-fit animate-pulse"
          >
            {/* Arrow Icon */}
            <StepBack />
  ...
  if (view === "lobby" && gameState) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 relative">
        <DarkAtmosphere />
        <SpectrumLogoBig />
        <div className="z-10 w-full max-w-lg bg-slate-900/95 backdrop-blur-xl p-8 rounded-2xl border border-fuchsia-900/30 shadow-2xl">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <div className="relative">
                  ...
                  {/* 3. The Copied Popup */}
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-fuchsia-500 text-white text-xs font-bold px-2 py-1 rounded shadow-lg animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {isHost && p.id !== gameState.hostId && (
                    <button
                      ...
                      className="p-1.5 text-red-600 hover:text-red-500 hover:bg-red-900/20 rounded-lg transition-all"
                      title="Kick Player"
                    >
                      <Trash2 size={16} />
                    </button>
