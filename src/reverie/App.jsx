...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "reverie";
const GAME_ID = "23";
...
const cardModules = import.meta.glob(
  "/src/reverie/assets/cards/*/*.{png,jpg,jpeg,webp,gif}",
  {
    eager: true,
    query: "?url",
    import: "default",
...
const DreamCard = ({
  ...
  const sizeClasses =
    size === "sm"
      ? "w-12 h-16 sm:w-16 sm:h-24 border sm:border-2"
      : size === "lg"
        ? "w-24 h-36 sm:w-40 sm:h-60 border-2 sm:border-4"
        : size === "xl"
          ? "w-36 h-52 sm:w-48 sm:h-72 border-4"
      ...
      <div className="bg-slate-900 border border-fuchsia-900/50 w-full max-w-4xl rounded-3xl shadow-2xl relative max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex border-b border-fuchsia-900/30 bg-slate-900 shrink-0">
          <button
            ...
            className={`flex-1 py-4 text-sm sm:text-base font-black tracking-widest uppercase transition-all flex items-center justify-center gap-2 ${tab === "rules" ? "text-fuchsia-400 border-b-2 border-fuchsia-400 bg-fuchsia-900/10" : "text-slate-500 hover:text-slate-300"}`}
          >
            <BookOpen className="w-4 h-4 sm:w-5 sm:h-5" /> Rules
          </button>
          <button
            onClick={() => setTab("gallery")}
            className={`flex-1 py-4 text-sm sm:text-base font-black tracking-widest uppercase transition-all flex items-center justify-center gap-2 ${tab === "gallery" ? "text-cyan-400 border-b-2 border-cyan-400 bg-cyan-900/10" : "text-slate-500 hover:text-slate-300"}`}
          >
            <ImageIcon className="w-4 h-4 sm:w-5 sm:h-5" /> Gallery
          </button>
          <button
            onClick={onClose}
            className="p-4 text-slate-500 hover:text-white transition-colors bg-slate-900 hover:bg-slate-800 border-l border-fuchsia-900/30"
          >
            <X size={24} />
          </button>
        </div>
            ...
            <div className="space-y-4 sm:space-y-6 text-xs sm:text-sm text-slate-300">
              <section>
                ...
                <p className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-fuchsia-900/30">
                  Be the first dreamer to reach <strong>30 points</strong>. You
                  score by giving clues that are neither too obvious nor too
                  obscure, and by tricking others into picking your cards.
                </p>
              </section>
              <section>
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 flex items-center gap-2">
                  <Eye className="text-fuchsia-400 w-5 h-5 sm:w-6 sm:h-6" /> The
                  Phases
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-fuchsia-900/30">
                    <strong className="text-fuchsia-300 block mb-1">
                      1. The Clue
                    </strong>
                    <p className="text-[10px] sm:text-xs">
                      The active <strong>Storyteller</strong> selects a card
                      from their hand and types a clue. It should relate to the
                      card.
                    </p>
                  </div>
                  <div className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-fuchsia-900/30">
                    ...
                    <p className="text-[10px] sm:text-xs">
                      All other players secretly select a card from their own
                      hand that best matches the Storyteller's clue.
                    </p>
                  </div>
                  <div className="bg-slate-800 p-3 sm:p-4 rounded-xl sm:col-span-2 border border-fuchsia-900/30">
                    <strong className="text-fuchsia-300 block mb-1">
                      3. The Gallery & The Vote
                    </strong>
                    <p className="text-[10px] sm:text-xs mb-2">
                      All chosen cards are shuffled and revealed. Every player
                      (except the Storyteller) votes on which card they believe
                      belongs to the Storyteller. You cannot vote for your own
                      card!
                    </p>
                  </div>
                </div>
              ...
              <section>
                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 flex items-center gap-2">
                  <Crown className="text-fuchsia-400 w-5 h-5 sm:w-6 sm:h-6" />{" "}
                  Scoring
                </h3>
                <ul className="bg-slate-800 p-3 sm:p-4 rounded-xl border border-fuchsia-900/30 list-disc pl-5 space-y-1 sm:space-y-2 text-[10px] sm:text-xs">
                  <li>
                    If <strong>everyone</strong> or <strong>no one</strong>{" "}
                    finds the Storyteller's card, Storyteller gets 0 points,
                    everyone else gets 2 points.
                  </li>
                  <li>
                    In any other case: The Storyteller gets{" "}
                    <strong>3 points</strong>. Players who guessed correctly get{" "}
                    <strong>3 points</strong>.
                  </li>
                  <li>
                    <strong>Bonus:</strong> Players (except the Storyteller) get{" "}
                    <strong>1 point</strong> for every vote their fake card
                    received!
                  </li>
                </ul>
              </section>
            ...
            <div className="flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
              {Object.keys(AVAILABLE_PACKS).length === 0 && (
                <p className="text-slate-500 text-center italic mt-10">
                  No image packs found in the assets folder.
                </p>
              )}
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {Object.keys(AVAILABLE_PACKS).map((packName) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="font-black text-white text-sm sm:text-base uppercase tracking-widest">
                        {packName}
                      </div>
                      <div className="text-xs text-cyan-500 font-bold">
                        {AVAILABLE_PACKS[packName].length} Cards
                      </div>
                    </div>
                  </button>
                ))}
            ...
            <div className="flex flex-col animate-in slide-in-from-right-8 duration-200">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-cyan-400 text-sm font-bold">
                    {AVAILABLE_PACKS[viewingPack].length} Images
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 sm:gap-4">
                {AVAILABLE_PACKS[viewingPack].map((card, idx) => (
                  <div
                    key={idx}
                    className="w-full aspect-[2/3] rounded-lg bg-cover bg-center shadow-md border border-white/10"
  ...
  <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-md flex items-center justify-center pt-16 sm:pt-20 pb-10 px-4">
    <div className="bg-slate-900 border border-fuchsia-900/50 w-full max-w-lg rounded-3xl shadow-2xl p-4 sm:p-6 relative flex flex-col max-h-full">
      ...
      <div className="shrink-0 mb-4 sm:mb-6">
        <h2 className="text-xl sm:text-2xl font-black text-center text-fuchsia-400 flex items-center justify-center gap-2">
          <BarChart2 className="w-5 h-5 sm:w-6 sm:h-6" /> Dreamer Standings
        </h2>
      </div>
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  {p.id === gameState.storytellerId && (
                    <BookOpen size={12} className="text-fuchsia-400 shrink-0" />
                  )}
                </div>
...
const SplashScreen = ({ onStart }) => {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("reverie_roomId");
    setHasSession(!!saved);
...
export default function ReverieGame() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("reverie_roomId") || "";
    }
    return "";
  });
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          if (!data.players.some((p) => p.id === user.uid)) {
            setRoomId("");
            localStorage.removeItem("reverie_roomId");
            setView("menu");
            setError("You have awoken.");
            return;
          }
          setGameState(data);
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (snap.exists() && snap.data().status === "lobby") {
        ...
        setRoomId(code);
        localStorage.setItem("reverie_roomId", code);
  ...
  const kickPlayer = async (targetId) => {
    ...
    try {
      ...
      await updateDoc(
        ...
        {
          players: newPlayers,
          logs: arrayUnion(triggerLog("A player awoke.", "warning")),
        },
      );
    } catch (e) {
  ...
  const submitClue = async () => {
    if (
      ...
      gameState.storytellerId !== user.uid
    )
      return;
    if (selectedCardIdx === null || !clueInput.trim())
      return alert("Select a card and enter a clue.");
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        table,
        clue: clueInput.trim(),
        turnPhase: "PLAY_CARDS",
        logs: arrayUnion(
          triggerLog(`Storyteller gave a clue: "${clueInput.trim()}"`),
        ),
      },
    );
  ...
  const playCard = async () => {
    ...
    if (allPlayed) {
      ...
      updates.logs = arrayUnion(
        triggerLog(
          "All cards submitted. Time to vote!",
          "important",
          true,
          "VOTING PHASE",
        ),
  ...
  const submitVote = async () => {
    ...
    const allVoted = players
      .filter((p) => p.id !== gameState.storytellerId)
      .every((p) => p.hasVoted);
    ...
    if (allVoted) {
      ...
      if (votesForSt === 0 || votesForSt === totalVoters) {
        logs.push(
          triggerLog(
            votesForSt === 0
              ? "No one found the Storyteller's card!"
              : "Everyone found the Storyteller's card! Too easy!",
            "warning",
            true,
            "SCORING",
          ),
        ...
        players.forEach((p) => {
          if (p.id !== gameState.storytellerId) p.score += 2;
        });
      } else {
        logs.push(
          triggerLog(
            "Some found the Storyteller's card!",
            "success",
            true,
            "SCORING",
          ),
        ...
        stPlayer.score += 3;
        stCard.votedBy.forEach((voterId) => {
          players.find((p) => p.id === voterId).score += 3;
        });
      }
  ...
  const nextRound = async () => {
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        turnPhase: "STORYTELLER_CLUE",
        clue: "",
        storytellerIdx: nextStIdx,
        storytellerId: players[nextStIdx].id,
        logs: arrayUnion(
          triggerLog(
            `A new round begins. ${players[nextStIdx].name} is the Storyteller.`,
            "neutral",
          ),
        ),
      },
        ...
        <a href={import.meta.env.BASE_URL}>
          <div className="flex items-center justify-center gap-3 mb-2">
            <div className="text-center pb-12 animate-pulse">
              <div className="inline-flex items-center gap-2 sm:gap-3 px-6 py-3 sm:px-8 sm:py-4 bg-slate-900/50 rounded-full border border-fuchsia-900/20 text-fuchsia-300 font-bold tracking-widest text-xs sm:text-sm uppercase backdrop-blur-sm">
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans select-none">
        ...
        <nav className="absolute top-0 left-0 w-full p-4 z-50">
          <a
            ...
            className="flex items-center gap-2 text-fuchsia-800 rounded-lg font-bold shadow-md hover:text-fuchsia-400 transition-colors w-fit animate-pulse text-sm sm:text-base"
          >
        ...
        <div className="z-10 text-center mb-8 sm:mb-10 mt-8">
          ...
          <p className="text-fuchsia-200/40 tracking-[0.3em] sm:tracking-[0.5em] uppercase mt-2 text-[10px] sm:text-xs">
            Dream. Describe. Deceive.
          </p>
        </div>
        <div className="bg-slate-900/80 backdrop-blur-md border border-fuchsia-900/30 p-6 sm:p-8 rounded-2xl w-full max-w-md shadow-2xl z-10 relative">
          ...
          <div className="space-y-4">
            <input
              className="w-full bg-black/50 border border-fuchsia-900 focus:border-fuchsia-500 p-3 sm:p-4 rounded-xl text-white outline-none transition-all text-base sm:text-lg font-bold text-center"
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                onClick={createRoom}
                disabled={loading}
                className="bg-gradient-to-br from-fuchsia-600 to-indigo-800 hover:from-fuchsia-500 hover:to-indigo-700 p-3 sm:p-4 rounded-xl font-bold flex flex-col items-center justify-center gap-1 transition-all active:scale-95 shadow-lg shadow-fuchsia-900/50"
              >
              ...
              <div className="flex flex-col gap-2">
                <input
                  className="bg-black/50 border border-fuchsia-900 focus:border-fuchsia-500 p-2 rounded-xl text-white text-center uppercase font-mono font-bold tracking-widest outline-none h-10 sm:h-12 text-sm sm:text-base"
                  placeholder="CODE"
                  value={roomCode}
                ...
                <button
                  onClick={joinRoom}
                  disabled={loading}
                  className="bg-slate-800 hover:bg-slate-700 p-2 rounded-xl font-bold text-slate-300 transition-all active:scale-95 h-full text-sm sm:text-base"
                >
                  Join
                </button>
              </div>
            ...
            <button
              ...
              className="w-full mt-4 text-fuchsia-500 hover:text-fuchsia-400 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 transition-colors py-2"
            >
  ...
  if (view === "lobby" && gameState) {
    ...
    const canStart =
      gameState.players.length >= 3 && gameState.players.length <= 6;
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 sm:p-6 relative">
        ...
        <div className="z-10 w-full max-w-lg bg-slate-900/90 backdrop-blur p-6 sm:p-8 rounded-2xl border border-fuchsia-900/30 shadow-2xl animate-in slide-in-from-bottom-8 mt-6">
          <div className="flex justify-between items-center mb-6 sm:mb-8 border-b border-slate-800 pb-4">
            <div>
              <h2 className="text-base sm:text-xl flex items-center gap-2 text-fuchsia-500 font-bold uppercase">
                <Sparkles className="w-5 h-5 sm:w-6 sm:h-6" /> Dream Code:
              </h2>
              <div className="flex items-center gap-2 sm:gap-3 mt-1">
                <div className="text-2xl sm:text-4xl font-mono text-white font-black">
                  {roomId}
                </div>
                <div className="relative">
                  ...
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-fuchsia-500 text-white text-[10px] sm:text-xs font-bold px-2 py-1 rounded shadow-lg animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-2 hover:bg-red-900/30 rounded text-rose-500 transition-colors"
              >
          ...
          <div className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
            <h3 className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
              Dreamers ({gameState.players.length}/6)
            </h3>
            {gameState.players.map((p) => (
              <div
                key={p.id}
                className="flex justify-between items-center bg-slate-800 p-3 sm:p-4 rounded-xl border border-slate-700"
              >
                <span className="font-bold flex items-center gap-2 sm:gap-3 text-sm sm:text-lg">
                  <div
                    className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full ${PLAYER_COLORS[p.colorIdx].bg} shadow-lg`}
                  />{" "}
                  {p.name}{" "}
                ...
                {gameState.hostId === user.uid && p.id !== user.uid && (
                  <button
                    ...
                    className="p-1.5 sm:p-2 bg-red-900/20 hover:bg-red-900/50 text-red-500 rounded-lg transition-colors border border-red-900/30"
                    title="Wake Player"
                  >
            ...
            {Array.from({ length: 6 - gameState.players.length }).map(
              (_, i) => (
                <div
                  key={i}
                  className="border-2 border-dashed border-slate-800 rounded-xl p-3 sm:p-4 flex items-center justify-center text-slate-600 font-bold uppercase text-xs sm:text-sm"
                >
                  Empty Slot
                </div>
              ),
          ...
          {isHost && Object.keys(AVAILABLE_PACKS).length > 0 && (
            <div className="mb-6 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
              <h4 className="text-[10px] sm:text-xs font-bold text-fuchsia-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                <ImageIcon className="w-4 h-4" /> Select Image Packs (1-3)
              </h4>
              <div className="flex flex-wrap gap-2">
                {Object.keys(AVAILABLE_PACKS).map((packName) => {
                  const isSelected =
                    gameState.selectedPacks?.includes(packName);
                  return (
                    <button
                      key={packName}
                    ...
                    >
                      ...
                      <span className="opacity-70 text-[10px]">
                        ({AVAILABLE_PACKS[packName].length})
                      </span>
                    </button>
                  );
                })}
          ...
          {isHost ? (
            <div className="flex flex-col gap-2">
              ...
              >
                <Sparkles className="w-5 h-5 sm:w-6 sm:h-6" /> Enter the Dream
              </button>
              {!canStart && (
                <div className="text-center text-[10px] sm:text-xs font-bold text-fuchsia-500 uppercase tracking-wider mt-1">
                  Requires 3 to 6 players to start
                </div>
              )}
            </div>
          ) : (
            <div className="text-center text-slate-500 text-xs sm:text-sm font-bold uppercase tracking-widest animate-pulse">
              Waiting for host...
            </div>
          )}
        </div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl max-w-xs w-full text-center shadow-2xl">
              ...
              <p className="text-slate-400 mb-6 text-xs sm:text-sm">
                {gameState.hostId === user.uid
                  ? "As Host, leaving awakes everyone."
                  : "You will disconnect from this session."}
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowLeaveConfirm(false)}
                  className="flex-1 bg-slate-800 hover:bg-slate-700 py-2 rounded font-bold text-slate-300 text-sm sm:text-base"
                >
                  Stay
                </button>
                <button
                  onClick={handleLeave}
                  className="flex-1 bg-rose-700 hover:bg-rose-600 py-2 rounded font-bold text-white text-sm sm:text-base"
                >
                  Leave
                </button>
              </div>
      ...
      <div className="fixed inset-0 bg-slate-950 text-white flex flex-col overflow-hidden font-sans select-none">
        ...
        {zoomCard && (
          <div className="fixed inset-0 z-[9999] bg-black/85 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200 pointer-events-none">
            <div
              ...
              style={{
                backgroundImage: zoomCard.image
                  ? `url(${zoomCard.image})`
                  : undefined,
              }}
            >
            ...
            <div className="bg-slate-900 border border-slate-700 p-6 rounded-xl max-w-xs w-full text-center shadow-2xl">
              ...
              {gameState.hostId === user.uid && (
                <button
                  ...
                  className="w-full bg-slate-800 hover:bg-slate-700 py-2 rounded font-bold text-rose-500 mt-2 text-xs sm:text-sm border border-slate-700 transition-colors"
                >
                  Return All to Lobby
                </button>
              )}
        ...
        <div className="h-14 sm:h-16 bg-slate-900 border-b border-fuchsia-900/50 flex items-center justify-between px-2 z-[160] shrink-0 shadow-lg relative">
          ...
          <div className="flex items-center gap-3 sm:gap-4 relative z-10">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-fuchsia-900/50 rounded-lg flex items-center justify-center border border-fuchsia-700 ml-1 sm:ml-2 shadow-[0_0_10px_rgba(217,70,239,0.3)]">
              <Sparkles className="text-fuchsia-400 w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <div>
              <div className="font-bold text-xs sm:text-sm tracking-wider text-fuchsia-100">
                REVERIE
              </div>
                    ...
                    <span className="text-fuchsia-400">
                      {
                        gameState.players.find(
                          (p) => p.id === gameState.storytellerId,
                        )?.name
                      }
                    </span>
                  </>
          ...
          <div className="flex items-center gap-1 sm:gap-2 relative z-10">
            <button
              ...
              className="p-1.5 sm:p-2 hover:bg-slate-800 rounded text-yellow-500 hover:text-white"
            >
              <BarChart2 className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={() => setShowGuide(true)}
              className="p-1.5 sm:p-2 hover:bg-slate-800 rounded text-slate-400 hover:text-white"
            >
              <BookOpen className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={() => setShowLogs(!showLogs)}
              className={`p-1.5 sm:p-2 rounded-full ${showLogs ? "bg-fuchsia-900 text-fuchsia-400" : "text-slate-400 hover:bg-slate-800"}`}
            >
              <History className="w-4 h-4 sm:w-5 sm:h-5" />
            </button>
            <button
              onClick={() => setShowLeaveConfirm(true)}
              className="p-1.5 sm:p-2 hover:bg-rose-900/30 rounded text-rose-500"
            >
        ...
        {showLogs && (
          <div className="fixed top-14 sm:top-16 right-2 sm:right-4 w-56 sm:w-64 max-h-56 sm:max-h-60 bg-slate-900/95 border border-slate-700 rounded-xl z-[155] overflow-y-auto p-2 shadow-2xl custom-scrollbar">
            <h4 className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 sticky top-0 bg-slate-900/95 py-1 sm:py-2">
              World History
            </h4>
            <div className="space-y-2">
              {gameState.logs
                ...
                .map((log) => (
                  <div
                    key={log.id}
                    className={`text-[10px] sm:text-xs p-2 rounded border-l-2 ${log.type === "success" ? "border-emerald-500 bg-emerald-900/10 text-emerald-200" : log.type === "warning" ? "border-amber-500 bg-amber-900/10 text-amber-200" : log.type === "failure" ? "border-rose-500 bg-rose-900/10 text-rose-200" : "border-fuchsia-500 bg-slate-800/30 text-slate-300"}`}
                  >
                    {log.text}
                  </div>
                ))}
            ...
            <div className="w-full flex justify-center mb-4 sm:mb-6 px-2 sm:px-4">
              {gameState.turnPhase === "STORYTELLER_CLUE" ? (
                <div className="text-center animate-pulse text-slate-400 uppercase tracking-widest font-bold border border-slate-800 bg-slate-900/50 py-2 px-4 sm:px-6 rounded-full shadow-lg text-[10px] sm:text-xs">
                  {isStoryteller
                    ? "Select a card and enter a clue"
                    : "Waiting for the Storyteller's Clue..."}
                </div>
              ) : (
                <div className="text-center border border-fuchsia-500/30 bg-fuchsia-900/20 py-3 sm:py-4 px-4 sm:px-8 rounded-2xl shadow-[0_0_30px_rgba(217,70,239,0.1)] backdrop-blur max-w-xs sm:max-w-2xl w-full mx-2">
                  <h3 className="text-[10px] sm:text-xs font-black text-fuchsia-400 uppercase tracking-widest mb-1 opacity-70">
                    The Clue
                  </h3>
                  <p className="text-xl sm:text-4xl font-serif text-white italic drop-shadow-md leading-tight break-words">
                    "{gameState.clue}"
                  </p>
                </div>
              )}
          ...
          <div className="flex flex-wrap justify-center items-center gap-4 sm:gap-6 w-full max-w-5xl mx-auto px-2 sm:px-4 mt-auto mb-auto">
            ...
            {gameState.turnPhase === "PLAY_CARDS" && (
              <>
                ...
                {Array.from({
                  length: gameState.players.length - gameState.table.length,
                }).map((_, idx) => (
                  <div
                    key={`empty-${idx}`}
                    className="w-24 h-36 sm:w-40 sm:h-60 rounded-xl border-2 sm:border-4 border-dashed border-slate-800/50 flex flex-col items-center justify-center text-slate-700 font-bold uppercase tracking-widest text-[10px] sm:text-xs"
                  >
                    Waiting
                  </div>
                ))}
            ...
            {gameState.turnPhase === "VOTING" &&
              gameState.table.map((entry, idx) => {
                ...
                const isSelectable =
                  !isStoryteller && !me.hasVoted && !isMyCard;
                ...
                return (
                  ...
                  >
                    <DreamCard
                      ...
                      onClick={() => {
                        if (isSelectable)
                          setSelectedTableIdx(isSelected ? null : idx);
                      }}
                    />
                    {isMyCard && (
        ...
        {gameState.turnPhase === "ROUND_END" && (
          <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex flex-col pt-16 pb-6 px-4 animate-in fade-in duration-300">
            {/* Header */}
            <div className="w-full max-w-5xl mx-auto text-center mb-6 shrink-0 px-2 sm:px-4">
              <h2 className="text-3xl sm:text-5xl font-black text-fuchsia-400 uppercase tracking-widest mb-2">
                The Truth Revealed
              </h2>
              <div className="text-center border border-fuchsia-500/30 bg-fuchsia-900/20 py-2 px-4 sm:px-6 rounded-2xl shadow-[0_0_30px_rgba(217,70,239,0.1)] inline-block max-w-full">
                <p className="text-lg sm:text-2xl font-serif text-white italic break-words whitespace-normal">
                  "{gameState.clue}"
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 w-full max-w-6xl mx-auto overflow-y-auto custom-scrollbar flex flex-col items-center">
              ...
              <div className="mb-10 w-full flex flex-col items-center">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-amber-400 font-black uppercase tracking-widest text-sm sm:text-base">
                    The Real Card
                  </span>
                </div>
                {(() => {
                  const stEntry = gameState.table.find(
                    (e) => e.playerId === gameState.storytellerId,
                  );
                  if (!stEntry) return null;
                  // Fallback if player left
                  const stPlayer = gameState.players.find(
                    (p) => p.id === stEntry.playerId,
                  ) || { name: "Departed Dreamer", colorIdx: 0 };
                  const ownerColor = PLAYER_COLORS[stPlayer.colorIdx];
                  const votedByPlayers = stEntry.votedBy
                    .map((vid) => gameState.players.find((p) => p.id === vid))
                    .filter(Boolean);
                  return (
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div className="flex flex-col items-center mt-2">
                        <span
                          className={`text-xs sm:text-sm font-black uppercase tracking-widest ${ownerColor.text}`}
                        >
                          {stPlayer.name}
                        </span>
                      </div>
              ...
              <div className="w-full flex flex-col items-center border-t border-white/10 pt-8">
                <span className="text-slate-500 font-bold uppercase tracking-widest text-xs sm:text-sm mb-6">
                  The Illusions
                </span>
                <div className="flex flex-wrap justify-center gap-6 sm:gap-10">
                  {gameState.table
                    .filter((e) => e.playerId !== gameState.storytellerId)
                    .map((entry, idx) => {
                      // Fallback if player left
                      const entryPlayer = gameState.players.find(
                        (p) => p.id === entry.playerId,
                      ) || { name: "Departed Dreamer", colorIdx: 0 };
                      const ownerColor = PLAYER_COLORS[entryPlayer.colorIdx];
                      const votedByPlayers = entry.votedBy
                        .map((vid) =>
                          gameState.players.find((p) => p.id === vid),
                        )
                        .filter(Boolean);
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="flex flex-col items-center bg-slate-900/80 px-4 py-2 rounded-xl border border-slate-700 min-w-[120px] text-center">
                            ...
                            <span
                              className={`text-xs font-black uppercase tracking-widest truncate w-full ${ownerColor.text}`}
                            >
                              {entryPlayer.name}
                            </span>
                          </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-slate-900/95 border-t-2 border-fuchsia-900/50 backdrop-blur-xl flex flex-col z-[60] shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex justify-between items-center px-2 sm:px-4 py-1.5 sm:py-2 border-b border-white/5 bg-black/20 shrink-0">
              <div className="flex items-center gap-2">
                <span
                  className={`text-[9px] sm:text-[11px] font-black uppercase tracking-widest ${gameState.turnPhase !== "ROUND_END" ? "text-fuchsia-400 animate-pulse" : "text-slate-500"}`}
                >
                  {gameState.turnPhase === "STORYTELLER_CLUE"
                    ? isStoryteller
                      ? "Select card & enter clue"
                      : "Waiting for Storyteller"
                    : gameState.turnPhase === "PLAY_CARDS"
                      ? me.hasPlayed
                        ? "Waiting for others"
                        : "Select card to match clue"
                      : gameState.turnPhase === "VOTING"
                        ? isStoryteller
                          ? "Waiting for votes"
                          : me.hasVoted
                            ? "Waiting for others to vote"
                            : "Vote for Storyteller's card"
                        : "Round Ended"}
                </span>
              </div>
              <div className="text-[9px] sm:text-[10px] font-bold text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded uppercase flex items-center gap-1">
                <Star className="w-3 h-3 sm:w-4 sm:h-4" /> My Score:{" "}
                {me?.score || 0}
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row p-2 sm:p-4 gap-2 sm:gap-4 items-center sm:items-end">
              ...
              <div className="w-full sm:w-64 shrink-0 flex flex-col justify-end gap-2 sm:border-l sm:border-white/10 sm:pl-4 mt-2 sm:mt-0">
                  ...
                  isStoryteller && (
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      <button
                        onClick={submitClue}
                        disabled={
                          selectedCardIdx === null || clueInput.trim() === ""
                        }
                ...
                {((gameState.turnPhase === "STORYTELLER_CLUE" &&
                  !isStoryteller) ||
                  (gameState.turnPhase === "PLAY_CARDS" &&
                    (isStoryteller || me.hasPlayed)) ||
                  (gameState.turnPhase === "VOTING" &&
                    (isStoryteller || me.hasVoted))) && (
                ...
                {gameState.players
                  ...
                  .map((p, i) => (
                    ...
                    >
                      ...
                      <span className="font-black text-yellow-400 text-sm sm:text-base shrink-0">
                        {p.score} pt
                      </span>
                    </div>
                  ))}
              ...
              {gameState.hostId === user.uid ? (
                <div className="shrink-0 pt-2">
                  <button
                    ...
                    className="bg-fuchsia-700 hover:bg-fuchsia-600 px-4 sm:px-6 py-3 sm:py-4 rounded-xl font-black w-full text-white transition-colors uppercase tracking-widest shadow-lg text-xs sm:text-sm"
                  >
                    Wake Everyone Up
                  </button>
                </div>
                ...
                <div className="shrink-0 pt-2">
                  <button
                    disabled
                    className="bg-slate-800 px-4 sm:px-6 py-3 sm:py-4 rounded-xl font-black w-full text-slate-600 uppercase tracking-widest text-xs sm:text-sm"
                  >
                    Waiting for Host...
                  </button>
                </div>
