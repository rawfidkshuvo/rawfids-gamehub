...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "emperor";
const GAME_ID = "4";
      ...
      return (
        <div
          ...
          style={{
            ...
            backgroundColor: Math.random() > 0.5 ? "#fffb00" : "#ffc400",
            animationDuration: `${10 + Math.random() * 20}s`,
            animationDelay: `${Math.random() * -20}s`,
            boxShadow: "0 0 12px 2px rgba(255, 234, 0, 0.8)", // Brighter glow
...
const FloatingBackground = React.memo(() => {
  ...
  const backgroundIcons = React.useMemo(() => {
    return [...Array(20)].map((_, i) => {
      const iconKeys = Object.keys(KINGS);
      const Icon = KINGS[iconKeys[i % iconKeys.length]].icon;
      return (
        <div
          key={i}
...
export default function EmperorGame() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("emperor_roomId") || "";
    }
    return "";
  });
  ...
  if (view === "lobby" && gameState) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 relative">
        ...
        <div className="z-10 w-full max-w-lg bg-slate-800/90 p-8 rounded-2xl border border-slate-700 shadow-2xl mb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <div className="relative">
                  ...
                  {/* 3. The Copied Popup */}
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded shadow-lg animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-3 mb-8">
            {gameState.players.map((p) => (
              <div
                key={p.id}
                className="flex items-center justify-between bg-slate-800 p-4 rounded border border-slate-700"
              >
                <span
                  className={`font-bold ${
    ...
    if (!opponent) {
      return (
        <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
          <DarkAtmosphere />
          <div className="bg-slate-800 p-8 rounded-xl border border-red-700 shadow-2xl relative z-10 max-w-md">
