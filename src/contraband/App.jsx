...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "contraband";
const GAME_ID = "12";
// --- Game Constants ---
...
const FloatingBackground = React.memo(() => {
  ...
  const backgroundIcons = React.useMemo(() => {
    return [...Array(20)].map((_, i) => {
      const iconKeys = Object.keys(GOODS);
      const Icon = GOODS[iconKeys[i % iconKeys.length]].icon;
      return (
        <div
          key={i}
...
export default function ContrabandGame() {
  ...
  const startGame = async () => {
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        marketTurnIndex: (firstInspectorIdx + 1) % players.length,
        currentRound: 1,
  ...
  const toggleReady = async () => {
    ...
    if (gameState.status === "playing" && gameState.turnState === "SHOPPING") {
      ...
      if (updatedPlayers.every((p) => p.ready)) {
        updatedPlayers.forEach((p) => (p.ready = false));
        await updateDoc(
          doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
          {
            players: updatedPlayers,
    ...
    // Standard toggle for other phases
    const players = gameState.players.map((p) =>
      p.id === user.uid ? { ...p, ready: !p.ready } : p,
    );

    // LOGIC 2: End of Round Summary
    if (
  ...
  const startNextRound = async () => {
    ...
    const nextPlayers = playersWithRoles.map((p) => {
      ...
      return { ...p, hand, loadedCrate: null, ready: false, hasRedrawn: false };
    });
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        marketTurnIndex: (nextInspectorIndex + 1) % gameState.players.length,
                    ...
                    <>
                      ...
                      {(!me.hasRedrawn && !isInspector) && (
                        <button
                          onClick={() => {
                            if (isDiscarding) {
                              if (selectedCards.length > 0) handleRedraw();
                              else setIsDiscarding(false);
                            } else {
                              setSelectedCards([]);
                          ...
                          className={`mt-2 w-full py-2 rounded text-xs border flex items-center justify-center gap-2 transition-colors ${
                            isDiscarding && selectedCards.length > 0
                              ? "bg-blue-900/40 hover:bg-blue-900/60 text-blue-400 border-blue-500/50"
                              : isDiscarding
                  ...
                  <div className="flex gap-2 pb-2">
                    {me.hand.map((cId, i) => (
                      <Card
                        ...
                        onClick={() => {
                          if (gameState.turnState === "LOADING") {
