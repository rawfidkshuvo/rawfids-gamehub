...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "immune";
const GAME_ID = "25";
  ...
  <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
      ...
      return (
        <div
          ...
          style={{
            ...
            boxShadow: "0 0 12px 2px rgba(0, 255, 200, 0.8)",
            "--driftX": driftX,
          }}
        />
      );
    ...
    <style>{`
      @keyframes float { 
        ...
        50% { transform: translate(var(--driftX), -60px) scale(1.5); opacity: 1; } 
      } 
      .animate-float { animation: float infinite ease-in-out; }
    `}</style>
  </div>
...
const GENERATE_DECK = () => {
  ...
  colors.forEach((c) => {
    ...
    for (let i = 0; i < count; i++)
      deck.push({ id: `c_${id++}`, type: "ORGAN", color: c });
  });
  colors.forEach((c) => {
    const count = c === "MULTI" ? 1 : 4;
    for (let i = 0; i < count; i++)
      deck.push({ id: `c_${id++}`, type: "VIRUS", color: c });
  });
  colors.forEach((c) => {
    const count = c === "MULTI" ? 4 : 4;
    for (let i = 0; i < count; i++)
      deck.push({ id: `c_${id++}`, type: "MEDICINE", color: c });
  });
  for (let i = 0; i < 2; i++)
    deck.push({ id: `c_${id++}`, type: "ACTION", action: "TRANSPLANT" });
  for (let i = 0; i < 3; i++)
    deck.push({ id: `c_${id++}`, type: "ACTION", action: "THIEF" });
  for (let i = 0; i < 2; i++)
    deck.push({ id: `c_${id++}`, type: "ACTION", action: "CONTAGION" });
...
const getCardVisuals = (card) => {
  ...
  if (card.type === "ACTION") {
    ...
    return {
      ...
      icon: icons[card.action] || Activity,
  ...
  const colors = {
    ...
    MULTI: {
      bg: "bg-gradient-to-br from-rose-500 via-emerald-500 to-blue-500",
      border: "border-white",
      icon: Sparkles,
      text: "text-white",
    },
  ...
  if (card.type === "MEDICINE")
    return { ...vis, icon: Pill, label: "MEDICINE" };
...
const GameCard = ({ card, size = "md", isSelected, isTargetable, onClick }) => {
  ...
  const sizeClasses =
    size === "xs"
      ? "w-6 h-8 sm:w-8 sm:h-12 border sm:border-2 shrink-0"
      : size === "sm"
        ? "w-10 h-14 sm:w-12 sm:h-16 border-2 shrink-0"
        : size === "md"
          ? "w-16 h-24 sm:w-20 sm:h-28 border-2 shrink-0"
  ...
  return (
    ...
    >
      ...
      {size !== "sm" && size !== "xs" && (
        <span
          className={`text-[6px] sm:text-[10px] font-black uppercase tracking-widest mt-0.5 sm:mt-1 ${visuals.text} drop-shadow-sm text-center px-1 leading-tight`}
        >
          {visuals.label}
        </span>
      )}
...
const OrganView = ({ organ, onClick, isTargetable, size = "md" }) => {
  ...
  return (
    <div
      className={`relative shrink-0 transition-all ${isTargetable ? "ring-2 sm:ring-4 ring-cyan-400 rounded-lg animate-pulse shadow-[0_0_15px_rgba(34,211,238,0.6)] cursor-pointer hover:scale-105" : ""}`}
    >
      <GameCard card={organ} size={size} onClick={onClick} />
      {organ.viruses.map((v, i) => (
        <div
          key={i}
          className={`absolute -bottom-1 -right-1 sm:-bottom-2 sm:-right-2 rotate-12 z-20 pointer-events-none drop-shadow-lg`}
        >
...
const FeedbackOverlay = ({ type, message, subtext, icon: Icon }) => (
  <div className="fixed inset-0 z-[300] flex items-center justify-center pointer-events-none animate-in fade-in zoom-in duration-200">
    <div
      className={`flex flex-col items-center justify-center p-4 sm:p-12 rounded-3xl border-2 sm:border-4 shadow-2xl backdrop-blur-xl max-w-[85vw] sm:max-w-xl mx-auto text-center ${type === "success" ? "bg-emerald-900/95 border-emerald-500 text-emerald-100" : type === "failure" ? "bg-rose-900/95 border-rose-500 text-rose-100" : "bg-cyan-900/95 border-cyan-500 text-cyan-100"}`}
    >
      {Icon && (
        <div className="mb-2 sm:mb-4 p-2 sm:p-4 bg-black/20 rounded-full shadow-inner">
          <Icon className="w-8 h-8 sm:w-16 sm:h-16 animate-bounce" />
        </div>
      )}
      <h2 className="text-xl sm:text-5xl font-black uppercase tracking-widest drop-shadow-md mb-1 sm:mb-2 leading-tight">
        {message}
      </h2>
      {subtext && (
        <p className="text-xs sm:text-xl font-bold opacity-90 tracking-wide">
          {subtext}
        </p>
      )}
    </div>
...
// --- FULLY REDESIGNED DETAILED HOW TO PLAY MODAL ---
const RulesModal = ({ onClose }) => (
  <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 animate-in fade-in zoom-in duration-200">
    <div className="bg-slate-900 border border-emerald-900/50 w-full max-w-5xl rounded-2xl sm:rounded-3xl shadow-2xl relative max-h-[95vh] flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 space-y-8 sm:space-y-12 text-slate-300">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="bg-blue-950/30 p-5 rounded-2xl border border-blue-900/50 shadow-inner">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-slate-900 p-5 rounded-2xl border border-white/10 shadow-inner flex flex-col gap-2">
              <div className="flex items-center gap-2 text-white font-black tracking-widest bg-black/40 p-2 rounded-lg w-fit">
                <HeartPulse className="w-5 h-5 text-cyan-400" /> ORGANS
              </div>
            ...
            {/* Viruses */}
            <div className="bg-rose-950/20 p-5 rounded-2xl border border-rose-900/30 shadow-inner flex flex-col gap-2">
            ...
            {/* Medicines */}
            <div className="bg-amber-950/20 p-5 rounded-2xl border border-amber-900/30 shadow-inner flex flex-col gap-2">
      ...
      {/* Footer */}
      <div className="shrink-0 p-4 sm:p-6 border-t border-white/10 bg-black/60 flex justify-center backdrop-blur-xl">
        <button
          onClick={onClose}
          className="w-full sm:w-auto px-12 py-3 sm:py-4 rounded-xl font-black text-sm sm:text-base shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all bg-emerald-600 hover:bg-emerald-500 text-white hover:scale-105 tracking-widest uppercase"
        >
          Return to Lab
        </button>
      </div>
...
const SplashScreen = ({ onStart }) => {
  ...
  useEffect(() => {
    setHasSession(!!localStorage.getItem("immune_roomId"));
    const img = new Image();
    img.src = CoverImage;
    img.onload = () => {
      setIsLoaded(true);
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-950 flex flex-col items-center justify-end pb-20 md:justify-center md:pb-0 font-sans overflow-hidden">
      ...
      >
        <div
          className={`w-full h-full bg-cover bg-center transition-transform duration-[2000ms] ease-out ${isLoaded ? "scale-100" : "scale-130"}`}
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
        <div
          className={`transform transition-all duration-1000 ease-out ${showButton ? "translate-y-0 opacity-100" : "translate-y-32 opacity-0"}`}
        >
          <button
            onClick={onStart}
            className="group relative px-12 py-5 bg-emerald-600/20 hover:bg-emerald-600/40 border border-emerald-500/50 hover:border-emerald-400 text-emerald-300 font-black text-2xl tracking-widest hover:scale-105 backdrop-blur-md overflow-hidden"
          >
...
export default function ImmuneGame() {
  ...
  const [view, setView] = useState(() =>
    sessionStorage.getItem("splashRefreshed") ? "menu" : "splash",
  );
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          if (!data.players.some((p) => p.id === user.uid)) {
            setRoomId("");
            localStorage.removeItem("immune_roomId");
            setView("menu");
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      setRoomId(newId);
      localStorage.setItem("immune_roomId", newId);
    } catch (e) {
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (snap.exists() && snap.data().status === "lobby") {
        ...
        setRoomId(code);
        localStorage.setItem("immune_roomId", code);
  ...
  const toggleGameMode = async () => {
    if (gameState.players.length === 4 || gameState.players.length === 6) {
      await updateDoc(
        doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
        {
          gameMode: gameState.gameMode === "team" ? "solo" : "team",
        },
      );
    }
  };
  ...
  const startGame = async () => {
    ...
    const isTeamValid =
      ...
      (gameState.players.length === 4 || gameState.players.length === 6);
    const actualGameMode = isTeamValid ? "team" : "solo";

    let players = gameState.players.map((p, i) => ({
      ...p,
      hand: [],
      body: [],
      skipTurn: false,
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        logs: arrayUnion(
          triggerLog(
            "The infection spreads. Game started!",
            "neutral",
            true,
            "START",
          ),
  ...
  const submitDiscard = async () => {
    ...
    logs.push(
      triggerLog(
        `${me.name} discarded ${selectedCards.length} ${selectedCards.length === 1 ? "card" : "cards"}.`,
        "neutral",
        true,
        "DISCARDED",
      ),
  ...
  const executeAction = async (actionType, params = null) => {
    ...
    try {
      if (actionType === "ORGAN") {
        const targetId = params?.targetPlayerId || me.id;
        const targetPlayer = state.players.find((p) => p.id === targetId);
        if (targetPlayer.body.some((o) => o.color === card.color))
          throw new Error("Target already has an organ of that color.");
        targetPlayer.body.push({ ...card, viruses: [], medicines: [] });
        me.hand.splice(cardIdx, 1);
        logs.push(
          triggerLog(
            `${me.name} placed a ${card.color} organ${targetId !== me.id ? ` for ${targetPlayer.name}` : ""}.`,
            "success",
            true,
            "NEW ORGAN",
          ),
        ...
        if (checkWinCondition(tPlayer))
          throw new Error("Cannot infect a secured body!");
        const tOrgan = tPlayer.body.find((o) => o.id === params.targetOrganId);
        if (tOrgan.medicines.length >= 2)
          throw new Error("That organ is immunized!");
        if (
          tOrgan.color !== card.color &&
          tOrgan.color !== "MULTI" &&
          card.color !== "MULTI"
        )
          throw new Error("Virus color must match the organ.");
        if (isTeamMode && tPlayer.teamId === me.teamId)
          throw new Error("Cannot infect teammate.");
        ...
        if (tOrgan.medicines.length === 1) {
          ...
          logs.push(
            triggerLog(
              `${me.name} destroyed a vaccine on ${tPlayer.name}'s organ.`,
              "warning",
              true,
              "VACCINE DESTROYED",
            ),
          ...
          logs.push(
            triggerLog(
              `${me.name} destroyed ${tPlayer.name}'s organ!`,
              "failure",
              true,
              "ORGAN DESTROYED",
            ),
          ...
          logs.push(
            triggerLog(
              `${me.name} infected ${tPlayer.name}'s organ.`,
              "warning",
              true,
              "INFECTION",
            ),
        ...
        const tOrgan = tPlayer.body.find((o) => o.id === params.targetOrganId);
        if (tOrgan.medicines.length >= 2)
          throw new Error("That organ is already immunized!");
        if (
          tOrgan.color !== card.color &&
          tOrgan.color !== "MULTI" &&
          card.color !== "MULTI"
        )
          throw new Error("Medicine color must match the organ.");
        ...
        if (tOrgan.viruses.length > 0) {
          ...
          logs.push(
            triggerLog(
              `${me.name} cured a virus on ${tPlayer.name}'s organ.`,
              "success",
              true,
              "CURED",
            ),
          ...
          if (tOrgan.medicines.length === 2)
            logs.push(
              triggerLog(
                `${me.name} immunized ${tPlayer.id !== me.id ? tPlayer.name + "'s " : "an "}organ!`,
                "success",
                true,
                "IMMUNIZED",
              ),
            ...
            logs.push(
              triggerLog(
                `${me.name} vaccinated ${tPlayer.id !== me.id ? tPlayer.name + "'s " : "an "}organ.`,
                "success",
                true,
                "VACCINATED",
              ),
        ...
        const pB = state.players.find(
          (p) => p.id === params.organB.targetPlayerId,
        );
        const oB = pB.body.find((o) => o.id === params.organB.targetOrganId);

        if (checkWinCondition(pA) || checkWinCondition(pB))
          throw new Error("Cannot transplant with a secured body!");
        if (pA.id === pB.id)
          throw new Error(
            "Transplants must happen between two different players.",
          );
        if (oA.medicines.length >= 2 || oB.medicines.length >= 2)
          throw new Error("Cannot transplant immunized organs!");

        const pABodyWithoutA = pA.body.filter((o) => o.id !== oA.id);
        const pBBodyWithoutB = pB.body.filter((o) => o.id !== oB.id);
        if (
          pABodyWithoutA.some(
            (o) => o.color === oB.color && oB.color !== "MULTI",
          )
        )
          throw new Error(`${pA.name} already has a ${oB.color} organ.`);
        if (
          pBBodyWithoutB.some(
            (o) => o.color === oA.color && oA.color !== "MULTI",
          )
        )
          throw new Error(`${pB.name} already has a ${oA.color} organ.`);
        ...
        logs.push(
          triggerLog(
            `${me.name} performed a transplant between ${pA.name} and ${pB.name}!`,
            "warning",
            true,
            "TRANSPLANT",
          ),
          ...
          if (myOrgan.viruses.length > 0) {
            ...
            for (let p of state.players) {
              ...
              let validOrgan = p.body.find(
                (o) =>
                  (o.color === topVirus.color ||
                    o.color === "MULTI" ||
                    topVirus.color === "MULTI") &&
                  o.medicines.length < 2 &&
                  o.viruses.length === 0,
              );
              if (validOrgan) {
                const v = myOrgan.viruses.pop();
                if (validOrgan.medicines.length === 1)
                  state.discardPile.push(v, validOrgan.medicines.pop());
        ...
        state.players.forEach((p) => {
          if (p.id !== me.id && (!isTeamMode || p.teamId !== me.teamId)) {
        ...
        logs.push(
          triggerLog(
            `${me.name} used Latex Glove! Enemies lost their hands.`,
            "failure",
            true,
            "LATEX GLOVE",
          ),
  ...
  const finishTurn = async (state, logs) => {
    ...
    let isTeamMode = state.gameMode === "team";
    ...
    const draw = (player) => {
      while (player.hand.length < 3) {
        if (state.deck.length === 0) {
          ...
          state.discardPile = [];
          logs.push(
            triggerLog("Deck reshuffled from discard pile.", "neutral"),
          );
        }
    ...
    let winningTeamOrPlayer = [];

    if (isTeamMode) {
      const teams = [...new Set(state.players.map((p) => p.teamId))];
      for (let tId of teams) {
        const teamPlayers = state.players.filter((p) => p.teamId === tId);
        if (teamPlayers.every(checkWinCondition)) {
          gameFinished = true;
          winningTeamOrPlayer = teamPlayers;
          break;
        }
      }
    } else {
      ...
      if (winner) {
        gameFinished = true;
        winningTeamOrPlayer = [winner];
      }
    }

    if (gameFinished) {
      state.status = "finished";
      winningTeamOrPlayer.forEach((w) => {
        const playerRef = state.players.find((x) => x.id === w.id);
        playerRef.wins += 1;
      });
      const names = winningTeamOrPlayer.map((w) => w.name).join(" & ");
      logs.push(
        triggerLog(
          `${names} completed the body!`,
          "important",
          true,
          isTeamMode ? "TEAM VICTORY!" : "VICTORY!",
        ),
      );
    } else {
      ...
      while (state.players[nextIdx].skipTurn) {
        ...
        logs.push(
          triggerLog(
            `${state.players[nextIdx].name} missed their turn due to Latex Glove.`,
            "warning",
            true,
            "TURN SKIPPED",
          ),
  ...
  const handleBoardClick = (targetPlayerId, targetOrganId) => {
    ...
    const card = gameState.players.find((p) => p.id === user.uid).hand[
      selectedCards[0]
    ];
    if (card.type === "VIRUS")
      executeAction("VIRUS", { targetPlayerId, targetOrganId });
    else if (card.type === "MEDICINE")
      executeAction("MEDICINE", { targetPlayerId, targetOrganId });
    else if (card.type === "ACTION" && card.action === "THIEF")
      executeAction("THIEF", { targetPlayerId, targetOrganId });
    else if (card.type === "ACTION" && card.action === "TRANSPLANT") {
      if (!transplantA) setTransplantA({ targetPlayerId, targetOrganId });
      else
        executeAction("TRANSPLANT", {
          organA: transplantA,
          organB: { targetPlayerId, targetOrganId },
        });
    }
  };
  ...
  const handlePlayerClick = (targetPlayerId) => {
    ...
    if (card.type === "ACTION" && card.action === "ERROR")
      executeAction("ERROR", { targetPlayerId });
  };
  ...
  const isTargetableForMe = (organ, me) => {
    if (selectedCards.length !== 1) return false;
    if (checkWinCondition(me)) return false; // LOCKED BODY
  ...
  const isPlayerTargetable = (playerId, me) => {
    if (selectedCards.length !== 1) return false;
    const tPlayer = gameState.players.find((p) => p.id === playerId);
    if (checkWinCondition(tPlayer)) return false; // LOCKED BODY

    const card = me.hand[selectedCards[0]];
    const isTeammate =
      gameState.gameMode === "team" && tPlayer.teamId === me.teamId;
    if (
      card.type === "ACTION" &&
      card.action === "ERROR" &&
      playerId !== me.id &&
        ...
        <a href={import.meta.env.BASE_URL}>
          <div className="flex items-center justify-center gap-3 mb-2">
            <div className="text-center pb-12 animate-pulse">
              <div className="inline-flex items-center gap-2 sm:gap-3 px-6 py-3 bg-slate-900/50 rounded-full border border-emerald-900/20 text-emerald-300 font-bold tracking-widest text-sm sm:text-base uppercase backdrop-blur-sm">
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans select-none">
        ...
        <nav className="absolute top-0 left-0 w-full p-4 z-50">
          <a
            ...
            className="flex items-center gap-2 text-emerald-800 rounded-lg font-bold shadow-md hover:text-emerald-400 transition-colors w-fit animate-pulse text-base sm:text-lg"
          >
        ...
        <div className="z-10 text-center mb-6 sm:mb-10 mt-6">
          ...
          <p className="text-emerald-200/40 tracking-[0.3em] sm:tracking-[0.5em] uppercase mt-1 sm:mt-2 text-xs sm:text-sm font-bold">
            Infect. Cure. Survive.
          </p>
        </div>
        <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-6 sm:p-8 rounded-3xl w-full max-w-md shadow-2xl z-10 relative">
          {error && (
            <div className="bg-red-900/20 border border-red-500/50 text-red-200 p-3 mb-4 rounded text-center text-sm font-bold flex items-center justify-center gap-2">
              <AlertTriangle className="w-5 h-5" /> {error}
            </div>
          )}
          <div className="space-y-4">
            <input
              className="w-full bg-black/50 border border-emerald-900/50 focus:border-emerald-400 p-4 rounded-xl text-white outline-none transition-all text-base sm:text-lg font-bold text-center shadow-inner"
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3 pt-2">
              <button
                onClick={createRoom}
                disabled={loading}
                className="bg-gradient-to-br from-emerald-500 to-teal-700 hover:from-emerald-400 hover:to-teal-600 p-4 rounded-xl font-bold flex flex-col items-center justify-center gap-1 transition-all active:scale-95 shadow-[0_0_20px_rgba(16,185,129,0.4)] border border-emerald-300/30"
              >
              ...
              <div className="flex flex-col gap-2">
                <input
                  className="bg-black/50 border border-emerald-900/50 focus:border-emerald-400 p-2 rounded-xl text-white text-center uppercase font-mono font-bold tracking-widest outline-none h-12 text-sm sm:text-base shadow-inner"
                  placeholder="CODE"
                  value={roomCode}
            ...
            <button
              ...
              className="w-full mt-2 sm:mt-4 text-emerald-500 hover:text-emerald-400 text-sm font-bold flex items-center justify-center gap-2 transition-colors py-2"
            >
  ...
  if (view === "lobby" && gameState) {
    ...
    const canStart =
      ...
      (gameState.gameMode !== "team" || isValidTeamCount);
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 sm:p-6 relative">
        ...
        <div className="z-10 w-full max-w-lg bg-slate-900/90 backdrop-blur-xl p-6 sm:p-8 rounded-3xl border border-white/10 shadow-2xl animate-in slide-in-from-bottom-8 mt-4 sm:mt-6">
          <div className="flex justify-between items-center mb-4 sm:mb-8 border-b border-white/10 pb-4">
            <div>
              <h2 className="text-lg sm:text-xl flex items-center gap-2 text-emerald-400 font-black uppercase tracking-widest">
                <FlaskConical className="w-6 h-6" /> Lab Code
              </h2>
              <div className="flex items-center gap-2 sm:gap-3 mt-1">
                <div className="text-3xl sm:text-4xl font-mono text-white font-black drop-shadow-md">
                  {roomId}
                </div>
                <div className="relative">
                  ...
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-emerald-500 text-white text-xs font-bold px-2 py-1 rounded shadow-[0_0_10px_rgba(16,185,129,0.5)] animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-8">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 flex justify-between items-center w-full">
            ...
            {gameState.players.map((p) => (
              ...
              >
                <span className="font-bold flex items-center gap-2 sm:gap-3 text-sm sm:text-lg">
                  <div
                    className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full ${PLAYER_COLORS[p.colorIdx].bg} shadow-md`}
                  />{" "}
                  {p.name}{" "}
          ...
          {isHost ? (
            <div className="flex flex-col gap-2 mt-4">
              {isValidTeamCount && (
                <div className="flex justify-center items-center gap-3 mb-2 bg-black/30 py-2 rounded-xl border border-white/5">
                  <span
                    className={`text-sm font-bold ${gameState.gameMode !== "team" ? "text-emerald-400" : "text-slate-500"}`}
                  >
                    SOLO
                  </span>
                  <button
                    onClick={toggleGameMode}
                    className="relative w-12 h-6 bg-slate-700 rounded-full flex items-center px-1 transition-all"
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full transition-all ${gameState.gameMode === "team" ? "translate-x-6" : ""}`}
                    />
                  </button>
                  <span
                    className={`text-sm font-bold ${gameState.gameMode === "team" ? "text-emerald-400" : "text-slate-500"}`}
  ...
  if (view === "game" && gameState) {
    ...
    const isMeCompleted = checkWinCondition(me);
    ...
    const renderActionBar = () => {
      ...
      if (selectedCards.length > 1)
        return (
          <button
            ...
            className="w-full bg-rose-600 hover:bg-rose-500 py-3 rounded-xl font-black tracking-widest text-white uppercase shadow-lg transition-all text-xs sm:text-sm"
          >
            Discard {selectedCards.length} Cards
          </button>
        );

      const c = me.hand[selectedCards[0]];
      const btnClasses =
        "flex-1 py-3 sm:py-4 rounded-xl font-black uppercase tracking-widest shadow-lg transition-all text-xs sm:text-sm flex justify-center items-center text-center leading-tight";

      if (c.type === "ORGAN") {
        const canPlaySelf =
          !isMeCompleted && !me.body.some((o) => o.color === c.color);
        const teammate =
          gameState.gameMode === "team"
            ...
            : null;
        const isTeammateCompleted = teammate
          ? checkWinCondition(teammate)
          : false;
        const canPlayTeammate =
          teammate && !isTeammateCompleted
            ? !teammate.body.some((o) => o.color === c.color)
            : false;
        ...
        return (
          <div className="flex gap-2 w-full flex-wrap sm:flex-nowrap">
            ...
            {canPlaySelf && (
              <button
                onClick={() =>
                  executeAction("ORGAN", { targetPlayerId: me.id })
                }
                className={`${btnClasses} bg-emerald-600 hover:bg-emerald-500 text-white`}
              >
                Play Self
              </button>
            )}
            {teammate && canPlayTeammate && (
              <button
                onClick={() =>
                  executeAction("ORGAN", { targetPlayerId: teammate.id })
                }
                className={`${btnClasses} bg-teal-600 hover:bg-teal-500 text-white`}
              >
                Play Teammate
              </button>
            )}
            {!canPlaySelf && (!teammate || !canPlayTeammate) && (
              <div
      ...
      if (c.type === "ACTION" && c.action === "CONTAGION")
        return (
          <div className="flex gap-2 w-full">
            ...
            <button
              onClick={() => executeAction("CONTAGION")}
              className={`${btnClasses} bg-fuchsia-600 hover:bg-fuchsia-500 text-white animate-pulse`}
            >
              Contagion
            </button>
          </div>
      ...
      if (c.type === "ACTION" && c.action === "TRANSPLANT")
        targetText = transplantA ? "Select 2nd Organ" : "Select 1st Organ";
      if (c.type === "ACTION" && c.action === "ERROR")
        targetText = "Select Opponent Profile";
      ...
      return (
        <div className="flex gap-2 w-full">
          ...
          <div
            className={`${btnClasses} bg-cyan-900/50 border border-cyan-500 text-cyan-300 animate-pulse`}
          >
            {targetText}
          </div>
        </div>
      ...
      <div className="fixed inset-0 bg-slate-950 text-white flex flex-col overflow-hidden font-sans select-none">
        ...
        <div className="h-14 sm:h-16 bg-slate-900/80 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-2 z-[160] shrink-0 shadow-lg relative">
          <div className="flex items-center gap-3 sm:gap-4 relative z-10">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-emerald-500/20 rounded-lg flex items-center justify-center border border-emerald-400/50 ml-1 sm:ml-2 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
              <HeartPulse className="text-emerald-400 w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <div>
          ...
          <div className="flex items-center gap-1 relative z-10">
            ...
            <button
              onClick={() => setShowLeaveConfirm(true)}
              className="p-2 hover:bg-rose-900/50 rounded-full text-rose-400 transition-colors ml-0.5"
            >
        ...
        {showLogs && (
          <div className="fixed top-14 sm:top-16 right-2 sm:right-4 w-56 sm:w-64 max-h-56 sm:max-h-60 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl z-[155] overflow-y-auto p-2 shadow-2xl custom-scrollbar">
            <h4 className="text-[10px] sm:text-xs font-black text-white uppercase tracking-widest mb-2 sticky top-0 bg-slate-900/95 py-2 border-b border-white/5">
              World History
            </h4>
        ...
        <div className="w-full bg-black/40 border-b border-white/5 flex overflow-x-auto custom-scrollbar p-3 sm:p-4 gap-4 shadow-inner shrink-0">
          {gameState.players
            ...
            .map((p) => {
              const isTurn =
                gameState.turnIndex ===
                gameState.players.findIndex((x) => x.id === p.id);
              const isTargetable = isPlayerTargetable(p.id, me);
              const isCompleted = checkWinCondition(p);
                ...
                >
                  {isCompleted && (
                    <div className="absolute inset-0 bg-emerald-900/30 backdrop-blur-[1px] z-30 flex items-center justify-center">
                      <span className="bg-black/90 px-3 py-1 rounded text-emerald-400 font-black tracking-widest text-xs rotate-[-15deg] border border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]">
                        BODY SECURE
                      </span>
                    </div>
                  )}
                  ...
                  >
                    <div className="flex flex-col">
                      <span
                        className={`text-[10px] sm:text-sm font-black text-white truncate max-w-[80px] sm:max-w-[120px] ${isTargetable ? "cursor-pointer" : ""}`}
                      >
                        {p.name}
                      </span>
                      {gameState.gameMode === "team" && (
                    ...
                    <span className="text-[8px] sm:text-[10px] font-bold bg-black/50 px-1.5 py-0.5 rounded text-yellow-400">
                      {p.wins} Wins
                    </span>
                  </div>
                  <div
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/10 flex flex-col z-[60] shadow-[0_-15px_30px_rgba(0,0,0,0.5)]">
          <div className="flex justify-between items-center px-4 py-2 border-b border-white/5 bg-black/30 shrink-0">
            <div className="flex items-center gap-2">
              <span className="text-[10px] sm:text-xs font-black text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-3 py-0.5 rounded-full uppercase drop-shadow-md">
                {me?.wins || 0} Wins
              </span>
            </div>
          ...
          <div className="flex flex-col p-3 sm:p-4 gap-3 sm:gap-4 justify-center items-center w-full max-w-3xl mx-auto">
            ...
            <div className="w-full">{renderActionBar()}</div>
          </div>
        </div>
              ...
              {gameState.hostId === user.uid ? (
                <div className="shrink-0 pt-2">
                  <button
                    ...
                    className="bg-gradient-to-br from-emerald-500 to-teal-700 hover:from-emerald-400 hover:to-teal-600 border border-emerald-300/50 px-6 py-4 rounded-2xl font-black w-full text-white transition-all uppercase tracking-widest shadow-[0_0_20px_rgba(16,185,129,0.4)] text-sm sm:text-base"
                  >
                    Play Again
                  </button>
                </div>
