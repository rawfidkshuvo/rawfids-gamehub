...
import CoverImage from "./assets/pirate_cover.png";
...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "pirates";
const GAME_ID = "5";
      ...
      return (
        <div
          ...
          style={{
            ...
            backgroundColor: Math.random() > 0.5 ? "#ff0000" : "#fc3b00",
            animationDuration: `${10 + Math.random() * 20}s`,
            animationDelay: `${Math.random() * -20}s`,
            boxShadow: "0 0 12px 2px rgba(219, 28, 28, 0.8)", // Brighter glow
...
export default function PiratesGame() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("pirates_roomId") || "";
    }
    return "";
  });
  ...
  const nextTurn = async (
    ...
    // RESET: Track points gained ONLY in this specific round for tie-breakers
    players.forEach((p) => (p.lastRoundCoins = 0));
    ...
    if (activePlayers.length === 1) {
      ...
      players[wIdx].coins += 1;
      players[wIdx].lastRoundCoins += 1;
      ...
      // --- CHECK WIN CONDITION & APPLY TIEBREAKERS ---
      const maxCoins = Math.max(...players.map((p) => p.coins));
      if (maxCoins >= winningGoal) {
        let candidates = players.filter((p) => p.coins === maxCoins);

        // TIEBREAKER 1: Most points in the last round
        if (candidates.length > 1) {
          const maxLast = Math.max(...candidates.map((p) => p.lastRoundCoins || 0));
          candidates = candidates.filter((p) => (p.lastRoundCoins || 0) === maxLast);
        }

        // TIEBREAKER 2: Turn order (Player closest to going last in the final round)
        if (candidates.length > 1) {
          const N = players.length;
          candidates.sort((a, b) => {
            const idxA = players.findIndex((p) => p.id === a.id);
            const idxB = players.findIndex((p) => p.id === b.id);
            const distA = (turnIndex - idxA + N) % N;
            const distB = (turnIndex - idxB + N) % N;
            return distA - distB; // Smaller distance wins
          });
        }
    ...
    if (deck.length === 0) {
      ...
      if (maxCoins >= winningGoal) {
        ...
        // TIEBREAKER 2: Turn order (Player closest to going last)
        if (candidates.length > 1) {
          const N = players.length;
          candidates.sort((a, b) => {
            const idxA = players.findIndex((p) => p.id === a.id);
      ...
      if (maxCoins >= winningGoal) {
        ...
        if (candidates.length > 1) {
          ...
          candidates.sort((a, b) => {
            const idxA = players.findIndex((p) => p.id === a.id);
            const idxB = players.findIndex((p) => p.id === b.id);
            const distA = (gameState.turnIndex - idxA + N) % N;
            const distB = (gameState.turnIndex - idxB + N) % N;
            return distA - distB; 
          });
        }
  ...
  if (view === "lobby" && gameState) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 relative">
        ...
        <div className="z-10 w-full max-w-lg bg-slate-900/90 backdrop-blur p-8 rounded-2xl border border-red-900/50 shadow-2xl mb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <div className="relative">
                  ...
                  {/* 3. The Copied Popup */}
                  {isCopied && (
                    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded shadow-lg animate-fade-in-up whitespace-nowrap">
                      Copied!
                    </div>
                  )}
                </div>
          ...
          <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-md flex flex-col items-center justify-center p-4 md:p-6 animate-in fade-in">
            <div className="bg-slate-900/90 p-6 md:p-8 rounded-3xl border border-yellow-500/50 shadow-[0_0_60px_rgba(234,179,8,0.3)] text-center animate-in zoom-in max-w-4xl w-full flex flex-col max-h-[90vh]">
              ...
              <div className="shrink-0 mb-6 border-b border-slate-800 pb-6">
                <Trophy size={64} className="text-yellow-400 mx-auto mb-4 animate-bounce drop-shadow-[0_0_20px_rgba(234,179,8,0.8)]" />
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-4 text-left pr-2 mb-6">
                {[...gameState.players]
                  .sort((a, b) => {
                    // Replicate the exact tie-breaking logic for the visual list
                    if (b.coins !== a.coins) return b.coins - a.coins;
                    if ((b.lastRoundCoins || 0) !== (a.lastRoundCoins || 0)) return (b.lastRoundCoins || 0) - (a.lastRoundCoins || 0);
                    
                    const N = gameState.players.length;
                    const idxA = gameState.players.findIndex(p => p.id === a.id);
                    const idxB = gameState.players.findIndex(p => p.id === b.id);
                    const distA = (gameState.turnIndex - idxA + N) % N;
                    const distB = (gameState.turnIndex - idxB + N) % N;
                    return distA - distB;
                  })
                  .map((p, i) => {
                    ...
                    return (
                      <div key={p.id} className={`p-4 rounded-xl border-2 flex flex-col gap-3 transition-all ${isWinner ? "bg-slate-800/80 border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.2)]" : "bg-slate-950/50 border-slate-800"}`}>
                        <div className="flex justify-between items-center border-b border-slate-700/50 pb-3">
                          <div className="flex items-center gap-3">
                            <span className={`font-mono text-lg font-black ${isWinner ? "text-yellow-500" : "text-slate-500"}`}>#{i + 1}</span>
                            <span className="font-black text-xl text-slate-200 uppercase tracking-widest flex items-center gap-2">
                              {p.name}
                              {isWinner && <Crown size={16} className="text-yellow-500" />}
                            </span>
                          </div>
                          <span className={`text-2xl font-black flex items-center gap-2 ${isWinner ? "text-yellow-400" : "text-gray-400"}`}>
                            {p.coins} <Coins size={20} />
                          </span>
                        </div>
                        <div>
                          <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                            <History size={14} /> Final Round History
                          </h4>
                          <div className="flex flex-wrap gap-2">
                            {p.playedCards && p.playedCards.length > 0 ? (
                                p.playedCards.map((c, idx) => (
                                    <CardDisplay key={idx} type={c} tiny />
                                ))
                            ) : (
                                <div className="text-slate-600 italic text-xs uppercase font-bold py-1">No cards played</div>
                            )}
                            {p.hand && p.hand.length > 0 && (
                                <div className="ml-4 pl-4 border-l border-slate-700 flex gap-2 items-center">
                                    <span className="text-xs text-slate-500 font-bold uppercase">Hand:</span>
                                    {p.hand.map((c, idx) => <CardDisplay key={`h-${idx}`} type={c} tiny />)}
                                </div>
                            )}
                          </div>
                        </div>
              ...
              <div className="shrink-0 pt-4 border-t border-slate-800">
                {gameState.hostId === user.uid ? (
                    <div className="flex gap-4">
                        <button onClick={restartGame} className="flex-1 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white px-8 py-4 rounded-xl text-lg font-black uppercase tracking-widest shadow-lg transition-transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2">
                            <RotateCcw size={20} /> Play Again
                        </button>
                        <button onClick={resetToLobby} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white px-8 py-4 rounded-xl text-lg font-black uppercase tracking-widest transition-transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2 border border-slate-700">
                            <Home size={20} /> Lobby
                        </button>
                    </div>
                ) : (
                  <button disabled className="bg-slate-800 text-slate-500 px-8 py-4 w-full rounded-xl text-lg font-black uppercase tracking-widest cursor-not-allowed border border-slate-700">
                    Waiting for Captain...
                  </button>
                )}
              </div>
