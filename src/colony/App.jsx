...
const APP_ID = typeof __app_id !== "undefined" ? __app_id : "colony";
const GAME_ID = "21";
      ...
      return (
        <div
          ...
          style={{
            ...
            backgroundColor: Math.random() > 0.5 ? "#ff6a00" : "#cd4810",
            animationDuration: `${10 + Math.random() * 20}s`,
            animationDelay: `${Math.random() * -20}s`,
            boxShadow: "0 0 12px 2px rgba(239, 149, 70, 0.8)", // Brighter glow
...
const DEV_CARD_TYPES = {
  KNIGHT: {
    id: "KNIGHT",
    name: "Knight",
    desc: "Move the robber. Play before rolling.",
    icon: Swords,
  },
  VP: {
    id: "VP",
    name: "Victory Point",
    desc: "Play to reveal and add +1 to your score.",
    icon: Trophy,
  },
  ROAD: {
    id: "ROAD",
  ...
  PLENTY: {
    id: "PLENTY",
    name: "Year of Plenty",
    desc: "Take any 2 resources.",
    icon: Gem,
  },
  MONOPOLY: {
    id: "MONOPOLY",
    name: "Monopoly",
    desc: "Steal all of 1 resource type.",
    icon: Crown,
  },
};
  ...
  gridShape.forEach(({ r, qStart, qEnd }) => {
    for (let q = qStart; q <= qEnd; q++) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (!edges[id].hexes.includes(`${q},${r}`))
          edges[id].hexes.push(`${q},${r}`);
      }
    }
  });
...
const FeedbackOverlay = ({ type, message, subtext, icon: Icon }) => (
  <div className="fixed inset-0 z-[300] flex items-center justify-center pointer-events-none animate-in fade-in zoom-in duration-300">
    <div
      className={`flex flex-col items-center justify-center p-8 md:p-12 rounded-3xl border-4 shadow-2xl backdrop-blur-xl max-w-sm md:max-w-xl mx-4 text-center ${
        type === "success"
  ...
  <div className="fixed inset-0 z-[200] bg-slate-950/95 backdrop-blur-md flex items-center justify-center pt-20 pb-10 px-4">
    <div className="bg-slate-900 border border-orange-900/50 w-full max-w-lg rounded-3xl shadow-2xl p-6 relative flex flex-col max-h-full">
      ...
      <div className="shrink-0 mb-6">
        <h2 className="text-2xl font-black text-center text-orange-400 flex items-center justify-center gap-2">
      ...
      <div className="space-y-4 overflow-y-auto custom-scrollbar flex-1 pr-2">
        {gameState.players
          .slice()
          .sort(
            (a, b) => getTotalScore(b, gameState) - getTotalScore(a, gameState),
          )
          .map((p) => {
              ...
              >
                <div className="flex justify-between items-center border-b border-slate-700 pb-2 mb-2">
                  <span
                    ...
                    style={{ color: PLAYER_COLORS[p.colorIdx].fill }}
                  >
                    {p.name}
                  </span>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-slate-400">
                  ...
                  <div className="flex justify-between text-slate-300">
                    ...
                    <span className="font-bold text-white">{p.roadLength}</span>
                  </div>
                  <div className="flex justify-between text-slate-300">
                    <span>Dev Cards (Hidden):</span>
                    <span className="font-bold text-white">
                  ...
                  {hasLongestRoad && (
                    <div className="col-span-2 flex justify-between text-orange-400 font-bold bg-orange-900/20 px-2 py-1 rounded mt-1">
                      <span className="flex items-center gap-1">
                        <Grip size={12} /> Longest Road
                      </span>
                      <span>+2</span>
                    </div>
                  )}
                  {hasLargestArmy && (
                    <div className="col-span-2 flex justify-between text-red-400 font-bold bg-red-900/20 px-2 py-1 rounded mt-1">
                      <span className="flex items-center gap-1">
                        <Swords size={12} /> Largest Army
                      </span>
                      <span>+2</span>
                    </div>
                  )}
...
const GameOverModal = ({ gameState, currentUserId, returnToLobby, onHide }) => {
  // Calculate final scores including hidden VPs for accurate maxScore
  const getFinalScore = (p) => {
    const publicScore = getTotalScore(p, gameState);
    const hiddenVP = (p.devCards?.["VP"] || 0) + (p.newDevCards?.["VP"] || 0);
    return publicScore + hiddenVP;
  };
      ...
      <div className="bg-slate-900 border border-orange-500/50 rounded-2xl w-full max-w-4xl flex flex-col shadow-[0_0_50px_rgba(249,115,22,0.2)] overflow-hidden max-h-[90vh] relative">
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-950 flex flex-col gap-4">
          <div className="flex flex-wrap gap-4 justify-center mt-2">
            <button
              onClick={onHide}
              className="px-6 py-3 rounded-xl font-bold flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white transition-all"
            >
              <Earth size={18} /> View Final Island
            </button>

            {isHost ? (
              <button
                onClick={returnToLobby}
                className="px-6 py-3 rounded-xl font-bold flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-orange-400 transition-all"
              >
                <StepBack size={18} /> Return All to Lobby
              </button>
            ) : (
              <button
                disabled
                className="px-6 py-3 rounded-xl font-bold flex items-center gap-2 bg-slate-900 text-slate-600 border border-slate-800 cursor-not-allowed"
              >
                Waiting for Host...
              </button>
            )}
...
export default function ColonyGame() {
  ...
  const [roomId, setRoomId] = useState(() => {
    ...
    if (sessionStorage.getItem("splashRefreshed")) {
      ...
      return localStorage.getItem("colony_roomId") || "";
    }
    return "";
  });
  ...
  const [requestTokens, setRequestTokens] = useState({
    WOOD: 0,
    BRICK: 0,
    WHEAT: 0,
    SHEEP: 0,
        ...
        if (diff !== 0) {
          ...
          setTimeout(() => {
            setResourceAnimations((curr) => {
              const updated = { ...curr };
              if (updated[res])
                updated[res] = updated[res].filter((a) => a.id !== id);
              return updated;
            });
          }, 2000);
        }
  ...
  const handleDiscardPhase = (data) => {
    if (data.turnPhase === "DISCARD") {
      ...
      const needsToDiscard =
        data.discardingPlayers && data.discardingPlayers.includes(user?.uid);
      if (needsToDiscard && !showDiscardModal) {
        const count = Object.values(currentPlayer.resources).reduce(
          (a, b) => a + b,
          0,
        );
  ...
  const handleGamePopups = (data) => {
    ...
    if (data.turnPhase === "ROBBER_STEAL" && data.turnIndex === idx) {
      ...
      const victims = [
        ...new Set(
          hexNodes
            .filter((n) => n.owner && n.owner !== user.uid)
            .map((n) => n.owner),
        ),
      ];
      if (victims.length > 0) {
  ...
  const getHexVertices = (q, r, allNodes) => {
    ...
    return Object.values(allNodes).filter(
      (node) => getDistance(center.x, center.y, node.x, node.y) < HEX_SIZE + 5,
    );
  };
  ...
  const startGame = async () => {
    setHideGameOverModal(false); // <-- Add here
    const board = GENERATE_BOARD();
    const desertEntry = Object.entries(board.hexes).find(
      ([_, h]) => h.resource === "DESERT",
    );
    ...
    try {
      ...
      await updateDoc(
        doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
        {
          players: newPlayers,
          logs: arrayUnion(
            triggerLog("A player was removed from the world.", "warning"),
          ),
        },
      );
    } catch (e) {
  ...
  const handleRollDice = async () => {
    ...
    if (total === 7) {
      logs.push(
        triggerLog(`The Robber strikes!`, "failure", true, "SEVEN ROLLED"),
      );
  ...
  const submitDiscard = async () => {
    const totalDiscarded = Object.values(discardTokens).reduce(
      (a, b) => a + b,
      0,
    );
    if (totalDiscarded !== mustDiscardAmount)
      return alert(`You must discard exactly ${mustDiscardAmount} cards.`);
    let valid = true;
    Object.keys(discardTokens).forEach((k) => {
      if (me.resources[k] < discardTokens[k]) valid = false;
    });
    if (!valid) return alert("You don't have those resources!");

    let players = JSON.parse(JSON.stringify(gameState.players));
    Object.keys(discardTokens).forEach((k) => {
      players[pIdx].resources[k] -= discardTokens[k];
    });
    let discardingPlayers = gameState.discardingPlayers.filter(
      (id) => id !== user.uid,
    );
  ...
  const handlePlaceRobber = async (q, r) => {
    if (gameState.turnIndex !== pIdx || gameState.turnPhase !== "ROBBER")
      return;
    const hexId = `${q},${r}`;
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        logs: arrayUnion(triggerLog(`Robber moved to ${hexId}.`)),
      },
    );
  };
  ...
  const getTradeRatios = (pId, board) => {
    ...
    Object.values(board.nodes).forEach((n) => {
      if (n.owner === pId && n.port) {
        if (n.port === "3:1")
          Object.keys(ratios).forEach(
            (k) => (ratios[k] = Math.min(ratios[k], 3)),
          );
        else {
          const res = n.port.split("_")[0];
          ratios[res] = 2;
        }
      }
    });
  ...
  const executeBankTrade = async () => {
    ...
    const requestedPicks = Object.values(requestTokens).reduce(
      (a, b) => a + b,
      0,
    );
    if (!validOffer)
      return alert(
        "Offer must be exactly in multiples of your bank/port ratios!",
      );
    if (requestedPicks !== allowedPicks || requestedPicks === 0)
      return alert(
        `You offered enough for ${allowedPicks} resources, but requested ${requestedPicks}.`,
      );

    Object.keys(offerTokens).forEach(
      (res) => (players[pIdx].resources[res] -= offerTokens[res]),
    );
    Object.keys(requestTokens).forEach(
      (res) => (players[pIdx].resources[res] += requestTokens[res]),
    );
    ...
    await updateDoc(
      ...
      {
        players,
        logs: arrayUnion(triggerLog(`${me.name} traded with the bank.`)),
      },
    );
  ...
  const proposeDomesticTrade = async () => {
    ...
    Object.keys(offerTokens).forEach((k) => {
      if (me.resources[k] < offerTokens[k]) valid = false;
    });
    if (!valid) return alert("You don't have those resources!");
  ...
  const respondToTrade = async (accept) => {
    ...
    if (accept) {
      ...
      Object.keys(active.request).forEach((k) => {
        if (me.resources[k] < active.request[k]) valid = false;
      });
      if (!valid)
        return alert("You don't have the resources to accept this trade.");

      let players = JSON.parse(JSON.stringify(gameState.players));
      const senderIdx = players.findIndex((p) => p.id === active.senderId);
      Object.keys(active.offer).forEach((k) => {
        players[senderIdx].resources[k] -= active.offer[k];
        players[pIdx].resources[k] += active.offer[k];
      });
      Object.keys(active.request).forEach((k) => {
        players[pIdx].resources[k] -= active.request[k];
        players[senderIdx].resources[k] += active.request[k];
      });
      ...
      await updateDoc(
        doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
        {
          ...
          logs: arrayUnion(
            triggerLog(
              `${me.name} accepted ${active.senderName}'s trade!`,
              "success",
            ),
          ),
        },
  ...
  const handleBuyDevCard = async () => {
    ...
    if (Object.keys(cost).some((k) => players[pIdx].resources[k] < cost[k]))
  ...
  const playDevCard = async (type) => {
    ...
    if (type !== "VP" && gameState.players[pIdx].hasPlayedDevCard)
      return alert("You can only play one Development Card per turn.");
    let players = JSON.parse(JSON.stringify(gameState.players));
    let updates = {};
    let logs = [];
    ...
    if (type === "VP") {
      ...
      if (!players[pIdx].usedDevCards)
        players[pIdx].usedDevCards = {
          KNIGHT: 0,
          VP: 0,
          ROAD: 0,
          PLENTY: 0,
      ...
      logs.push(
        triggerLog(
          `${players[pIdx].name} revealed ${vpCount} Victory Point Card${vpCount > 1 ? "s" : ""}!`,
          "important",
          true,
        ),
      );
      ...
      logs.push(
        triggerLog(
          `${players[pIdx].name} played a ${DEV_CARD_TYPES[type].name} Card!`,
          "important",
          true,
        ),
      );
      ...
      if (type === "KNIGHT") {
        ...
        if (players[pIdx].playedKnights > currentArmySize) {
          updates.largestArmy = {
            playerId: user.uid,
            size: players[pIdx].playedKnights,
          };
          if (currentArmyHolder !== user.uid)
            logs.push(
              triggerLog(
                `${players[pIdx].name} took the Largest Army!`,
                "important",
                true,
              ),
            );
    ...
    if (
      checkVictory(
        ...
        updates.largestArmy || gameState.largestArmy,
      )
    ) {
      updates.status = "finished";
      logs.push(
        triggerLog(
          `${players[pIdx].name} wins the game!`,
          "important",
          true,
          "VICTORY",
        ),
  ...
  const handleYearOfPlenty = async (res1, res2) => {
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        turnPhase: nextPhase,
        logs: arrayUnion(
          triggerLog(`${me.name} took ${res1} and ${res2} via Year of Plenty.`),
        ),
      },
    );
    setPopupContent(null);
  ...
  const handleMonopoly = async (res) => {
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        turnPhase: nextPhase,
        logs: arrayUnion(
          triggerLog(`${me.name} stole ${totalStolen} ${res} via Monopoly!`),
        ),
      },
    );
    setPopupContent(null);
  ...
  const isValidSettlement = (nodeId) => {
    ...
    const tooClose = Object.values(gameState.board.nodes).some(
      (n) => n.owner && getDistance(n.x, n.y, node.x, node.y) < HEX_SIZE + 5,
    );
    if (tooClose) return false;
    if (gameState.turnPhase.startsWith("SETUP_")) return true;
    const hasRoad = Object.values(gameState.board.edges).some(
      (e) =>
        e.owner === user.uid &&
        getDistance(e.x, e.y, node.x, node.y) < HEX_SIZE / 2 + 5,
    );
    return hasRoad;
  };
  ...
  const isValidRoad = (edgeId) => {
    ...
    const connectedToMyNode = Object.values(gameState.board.nodes).some(
      (n) =>
        n.owner === user.uid &&
        getDistance(n.x, n.y, edge.x, edge.y) < HEX_SIZE / 2 + 5,
    );
    const myRoads = Object.values(gameState.board.edges).filter(
      (e) => e.owner === user.uid,
    );
    const validConnectedRoad = myRoads.some((myRoad) => {
      const sharedNodeId = [myRoad.n1, myRoad.n2].find(
        (n) => n === edge.n1 || n === edge.n2,
      );
      if (sharedNodeId) {
  ...
  const handleBuildNode = async (nodeId) => {
    ...
    if (
      ...
      if (!gameState.turnPhase.startsWith("SETUP_")) {
        ...
        if (Object.keys(cost).some((k) => players[pIdx].resources[k] < cost[k]))
          return alert("Not enough resources!");
        Object.keys(cost).forEach(
          (k) => (players[pIdx].resources[k] -= cost[k]),
        );
      }
      ...
      players.forEach((p) => {
        ...
        if (pRoadLen > maxRoadLength) {
          maxRoadLength = pRoadLen;
          maxPlayers = [p.id];
        } else if (pRoadLen === maxRoadLength && pRoadLen > 4) {
      ...
      if (maxPlayers.length === 1) currentLongestPlayer = maxPlayers[0];
        ...
        if (
          maxPlayers.includes(previousOwner) &&
          maxRoadLength === previousLength
        )
          currentLongestPlayer = previousOwner;
      ...
      if (
        ...
        updates.longestRoad = {
          playerId: currentLongestPlayer,
          length: maxRoadLength,
        };
        if (currentLongestPlayer) {
          if (currentLongestPlayer !== previousOwner) {
            const newOwnerName = players.find(
              (p) => p.id === currentLongestPlayer,
            ).name;
            logs.push(
              triggerLog(
                `Longest road updated! ${newOwnerName} holds it.`,
                "important",
                true,
              ),
            );
          ...
          logs.push(
            triggerLog(
              `The Longest Road has been broken! No one holds it.`,
              "important",
              true,
            ),
          );
          ...
          const adjHexes = Object.values(board.hexes).filter(
            (h) =>
              getDistance(
                h.center.x,
                h.center.y,
                board.nodes[nodeId].x,
                board.nodes[nodeId].y,
              ) <
              HEX_SIZE + 5,
          );
          adjHexes.forEach((h) => {
            if (h.resource !== "DESERT") players[pIdx].resources[h.resource]++;
          });
        }
    ...
    if (
      checkVictory(
        players,
        updates.longestRoad || gameState.longestRoad,
        gameState.largestArmy,
      )
    ) {
      updates.status = "finished";
      logs.push(
        triggerLog(`${me.name} wins the game!`, "important", true, "VICTORY"),
      );
    }
  ...
  const handleBuildEdge = async (edgeId) => {
    ...
    if (
      !gameState.turnPhase.includes("SETUP_") &&
      !gameState.turnPhase.includes("ROAD_BUILDING")
    ) {
    ...
    if (currentLen > currentRecord) {
      ...
      if (currentHolder !== user.uid)
        logs.push(
          triggerLog(`${me.name} took the Longest Road!`, "important", true),
        );
    }
    ...
    if (gameState.turnPhase === "SETUP_ROAD") {
      ...
      if (gameState.setupRound === 1) {
        if (gameState.turnIndex === numPlayers - 1) {
          updates.setupRound = 2;
          updates.turnPhase = "SETUP_SETTLEMENT";
        } else {
          updates.turnIndex = gameState.turnIndex + 1;
          updates.turnPhase = "SETUP_SETTLEMENT";
        }
      } else {
        if (gameState.turnIndex === 0) {
          ...
          logs.push(
            triggerLog(
              "Setup complete. Main phase begins!",
              "important",
              true,
              "GAME START",
            ),
    ...
    } else if (gameState.turnPhase === "ROAD_BUILDING_1") {
      updates.turnPhase = "ROAD_BUILDING_2";
    } else if (gameState.turnPhase === "ROAD_BUILDING_2") {
      updates.turnPhase = gameState.hasRolled ? "MAIN" : "ROLL";
    }
  ...
  const skipRoadBuilding = async () => {
    let nextPhase =
      gameState.turnPhase === "ROAD_BUILDING_1"
        ? "ROAD_BUILDING_2"
        : gameState.hasRolled
          ? "MAIN"
          : "ROLL";
  ...
  const handleEndTurn = async () => {
    if (gameState.turnIndex !== pIdx || gameState.turnPhase !== "MAIN") return;
    let players = JSON.parse(JSON.stringify(gameState.players));
    Object.keys(players[pIdx].newDevCards).forEach((k) => {
      players[pIdx].devCards[k] += players[pIdx].newDevCards[k];
      players[pIdx].newDevCards[k] = 0;
    });
    ...
    await updateDoc(
      doc(db, "artifacts", APP_ID, "public", "data", "rooms", roomId),
      {
        ...
        hasRolled: false,
        logs: arrayUnion(
          triggerLog(`Turn passed to ${gameState.players[nextIdx].name}.`),
        ),
      },
    );
    setActiveBuildMode(null);
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-4 text-center">
        ...
        <div className="bg-orange-500/10 p-8 rounded-2xl border border-orange-500/30">
          ...
          <p className="text-gray-400">
            The island is currently undergoing terraforming. Please return soon!
          </p>
        </div>
  ...
  if (!user)
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center text-orange-500 animate-pulse">
        Colonizing...
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans select-none">
        ...
        <div className="z-10 text-center mb-10 mt-8">
          <Home
            size={64}
            className="text-orange-400 mx-auto mb-4 animate-bounce"
          />
        ...
        <div className="bg-slate-900/80 backdrop-blur-md border border-orange-500/30 p-8 rounded-2xl w-full max-w-md shadow-2xl z-10 relative">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full mt-4 text-orange-400 hover:text-orange-300 text-sm font-bold flex items-center justify-center gap-2 transition-colors py-2"
            >
              <BookOpen size={16} /> Settler's Guide
            </button>
          </div>
        </div>
      </div>
            ...
            {gameState.players.map((p) => (
              ...
              >
                <span className="font-bold flex items-center gap-3 text-lg">
                  <div
                    className={`w-4 h-4 rounded-full ${PLAYER_COLORS[p.colorIdx].bg}`}
                  />{" "}
                  {p.name}{" "}
            ...
            <div className="bg-slate-900 border border-slate-700 p-6 rounded-xl max-w-xs w-full text-center shadow-2xl">
              ...
              <p className="text-slate-400 mb-6 text-sm">
                {gameState.hostId === user.uid
                  ? "As Host, leaving ends the session for everyone."
                  : "You will disconnect from this session."}
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowLeaveConfirm(false)}
                  className="flex-1 bg-slate-800 hover:bg-slate-700 py-2 rounded font-bold text-slate-300"
                >
                  Stay
                </button>
                <button
                  onClick={handleLeave}
                  className="flex-1 bg-red-600 hover:bg-red-500 py-2 rounded font-bold text-white"
                >
                  Leave
                </button>
              </div>
  ...
  if (view === "game" && gameState) {
    ...
    const getHexClasses = (hId) => {
      if (gameState.turnPhase === "ROBBER" && isMyTurn)
        return "cursor-pointer hover:scale-105 hover:brightness-125 z-20";
      return "opacity-100";
    };
    ...
    const getPhaseDisplayName = (phase) => {
      const phaseMap = {
        ...
        DISCARD: "Players Discarding Extra",
        ROBBER_STEAL: "Steal from a Player",
        SETUP_SETTLEMENT: "Place a Settlement",
        SETUP_ROAD: "Place a Road",
      };
      ...
      <div className="fixed inset-0 bg-slate-950 text-white flex flex-col overflow-hidden font-sans select-none">
        ...
        {showDiscardModal && me && (
          <div className="fixed inset-0 z-[200] bg-black/90 flex items-center justify-center p-4 backdrop-blur-md">
            <div className="bg-slate-900 border-2 border-red-500 p-6 rounded-2xl max-w-sm w-full text-center shadow-[0_0_50px_rgba(239,68,68,0.3)] relative">
              ...
              <p className="text-red-300 text-sm mb-6 font-bold">
                You have too many cards. Discard exactly {mustDiscardAmount}.
              </p>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div
                            className={`p-1.5 rounded ${RESOURCES[res].color} w-8 flex justify-center`}
                          >
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() =>
                              setDiscardTokens({
                                ...discardTokens,
                                [res]: Math.max(0, discardTokens[res] - 1),
                              })
                            }
                            className="w-8 h-8 rounded-full bg-slate-700 hover:bg-slate-600 font-bold"
                          >
                            -
                          </button>
                          <span className="w-4 text-center font-black text-red-400">
                            {discardTokens[res]}
                          </span>
                          <button
                            onClick={() =>
                              setDiscardTokens({
                                ...discardTokens,
                                [res]: Math.min(count, discardTokens[res] + 1),
                              })
                            }
                            className="w-8 h-8 rounded-full bg-slate-700 hover:bg-slate-600 font-bold"
                          >
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <span
                  className={`text-xl font-black ${Object.values(discardTokens).reduce((a, b) => a + b, 0) === mustDiscardAmount ? "text-orange-500" : "text-red-500"}`}
                >
                  {Object.values(discardTokens).reduce((a, b) => a + b, 0)} /{" "}
                  {mustDiscardAmount}
                </span>
              </div>
              <button
                ...
                className="w-full py-3 bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:hover:bg-red-600 rounded-xl font-black text-white transition-all uppercase tracking-widest shadow-lg"
              >
                Confirm Discard
              </button>
            </div>
              ...
              <div className="flex flex-col gap-2">
                {popupContent.victims.map((vId) => {
                  ...
                  return (
                    <button
                      key={v.id}
                      onClick={() => executeSteal(v.id)}
                      className="py-3 bg-slate-800 hover:bg-red-900/50 rounded-xl font-bold border border-slate-600 transition-all"
                    >
                      {v.name}
                    </button>
                  );
              ...
              <div className="bg-black/30 p-4 rounded-xl mb-6">
                ...
                <div className="flex justify-center gap-2 mb-4">
                  {Object.keys(popupContent.trade.offer).map(
                    (k) =>
                      popupContent.trade.offer[k] > 0 && (
                        <span
                          key={k}
                          className="bg-orange-900/50 px-2 py-1 rounded text-xs border border-orange-800"
                        >
                          {popupContent.trade.offer[k]} {k}
                        </span>
                      ),
                  )}
                </div>
                ...
                <div className="flex justify-center gap-2">
                  {Object.keys(popupContent.trade.request).map(
                    (k) =>
                      popupContent.trade.request[k] > 0 && (
                        <span
                          key={k}
                          className="bg-red-900/50 px-2 py-1 rounded text-xs border border-red-800"
                        >
                          {popupContent.trade.request[k]} {k}
                        </span>
                      ),
                  )}
                </div>
          ...
          <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4">
            <div className="bg-slate-900 border-2 border-yellow-500 p-6 rounded-2xl max-w-sm w-full text-center shadow-2xl">
              ...
              <p className="text-slate-300 text-sm mb-4">
                Select two resources to add to your hand.
              </p>
              <div className="grid grid-cols-5 gap-2 mb-6">
                {["WOOD", "BRICK", "SHEEP", "WHEAT", "ORE"].map((res) => (
                  <button
                    key={res}
                    onClick={() => {
                      if (!popupContent.res1)
                        setPopupContent({ ...popupContent, res1: res });
                      else handleYearOfPlenty(popupContent.res1, res);
                    }}
                    className={`p-2 rounded-lg border-2 flex justify-center items-center ${popupContent.res1 === res ? "border-yellow-500 bg-yellow-900/50" : "border-slate-700 hover:border-slate-500"}`}
                  >
                    {React.createElement(RESOURCES[res].icon, {
                      size: 24,
        ...
        {popupContent && popupContent.type === "MONOPOLY" && (
          <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4">
            <div className="bg-slate-900 border-2 border-purple-500 p-6 rounded-2xl max-w-sm w-full text-center shadow-2xl">
              ...
              <p className="text-slate-300 text-sm mb-4">
                Declare a resource. All players must give you all of that
                resource.
              </p>
              <div className="grid grid-cols-5 gap-2 mb-6">
                {["WOOD", "BRICK", "SHEEP", "WHEAT", "ORE"].map((res) => (
                  <button
                    ...
                    className="p-2 rounded-lg border-2 border-slate-700 flex justify-center items-center hover:border-purple-500 hover:bg-purple-900/30"
                  >
                    {React.createElement(RESOURCES[res].icon, {
                      size: 24,
                      className: "text-white",
        ...
        <div className="h-14 md:h-16 bg-slate-900 border-b border-orange-900/30 flex items-center justify-between px-2 z-[160] shrink-0 shadow-lg">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="font-bold text-sm tracking-wider text-orange-100">
                COLONY
              </div>
            ...
            <div
              className={`flex items-center gap-1.5 bg-slate-800 p-1.5 rounded-lg border border-slate-700 shadow-inner ml-2 ${flashDice ? "animate-flash-red" : ""}`}
            >
              <div
                className={`w-6 h-6 rounded flex items-center justify-center font-black text-sm shadow-inner ${gameState.dice[0] + gameState.dice[1] === 7 ? "bg-red-500 text-white" : "bg-slate-200 text-slate-900"}`}
              >
                {gameState.dice[0]}
              </div>
              <div
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2 rounded-full ${showLogs ? "bg-orange-900 text-orange-400" : "text-gray-400 hover:bg-gray-800"}`}
            >
              <History size={18} />
            </button>
            <button
            ...
            <div className="space-y-2">
              {gameState.logs
                ...
                .map((log) => (
                  <div
                    key={log.id}
                    className={`text-xs p-2 rounded border-l-2 ${log.type === "success" ? "border-orange-500 bg-orange-900/10" : log.type === "warning" ? "border-amber-500 bg-amber-900/10" : log.type === "failure" ? "border-red-500 bg-red-900/10" : "border-slate-500 bg-slate-800/30"}`}
                  >
                    {log.text}
                  </div>
                ))}
          ...
          <div className="absolute top-2 md:top-4 left-0 w-full flex flex-col items-center gap-2 z-[100] pointer-events-none px-2 md:px-4">
            <div className="flex flex-row flex-wrap justify-center gap-1 md:gap-4 w-full">
              {gameState.players.map((p, i) => (
                <div
                  key={p.id}
                  className={`flex flex-col items-center px-2 py-1 md:px-4 md:py-2 rounded-xl border-2 pointer-events-auto backdrop-blur-md bg-slate-900/80 transition-all ${gameState.turnIndex === i ? PLAYER_COLORS[p.colorIdx].border + " shadow-[0_0_15px_" + PLAYER_COLORS[p.colorIdx].fill + "]" : "border-transparent opacity-70"}`}
                >
                  <span
                    className="text-[10px] md:text-sm font-black uppercase tracking-widest drop-shadow-md truncate max-w-[60px] md:max-w-[120px]"
                    style={{ color: PLAYER_COLORS[p.colorIdx].fill }}
                  >
                    {p.name}
                  </span>
                  <div className="flex gap-1 md:gap-3 items-center mt-0.5 md:mt-1">
                    <span className="text-[10px] md:text-sm font-black text-white flex items-center gap-0.5 md:gap-1">
                      <Trophy
                        size={10}
                        className="md:w-3.5 md:h-3.5 text-yellow-500"
                      />
                      {getTotalScore(p, gameState)}
                    </span>
                    {gameState.longestRoad?.playerId === p.id && (
                      <Grip
                        size={10}
                        className="md:w-3.5 md:h-3.5 text-orange-400"
                        title="Longest Road"
                      />
                    )}
                    {gameState.largestArmy?.playerId === p.id && (
                      <Swords
                        size={10}
                        className="md:w-3.5 md:h-3.5 text-red-400"
                        title="Largest Army"
                      />
                    )}
                    <span
                      className="text-[10px] md:text-sm font-black text-white flex items-center gap-0.5 md:gap-1 opacity-80"
                      title="Dev Cards"
                    >
                      <Scroll size={10} className="md:w-3.5 md:h-3.5" />
                      {Object.values(p.devCards).reduce((a, b) => a + b, 0) +
                        Object.values(p.newDevCards).reduce((a, b) => a + b, 0)}
                    </span>
                  </div>
            ...
            <div className="md:hidden pointer-events-auto bg-slate-900/90 backdrop-blur-md border border-slate-700 px-4 py-1.5 rounded-full shadow-lg mt-1">
              <span
                className={`text-[11px] font-black uppercase tracking-widest ${isMyTurn ? "text-orange-400 animate-pulse" : "text-slate-400"}`}
              >
                {isMyTurn
          ...
          <div className="relative transform scale-[0.75] sm:scale-[0.85] md:scale-[1.15] transition-transform mt-20 md:mt-0">
            {Object.entries(gameState.board.hexes).map(([hId, h]) => {
              ...
              return (
                <div
                  key={hId}
                  onClick={() => handlePlaceRobber(h.q, h.r)}
                  className={`absolute transition-all duration-300 ${getHexClasses(hId)}`}
                  style={{
                    left: h.center.x,
                    top: h.center.y,
                ...
                >
                  <div
                    className={`w-full h-full absolute ${ResDef.color} opacity-80`}
                    style={{
                      clipPath:
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-20">
                    ...
                    {h.number && (
                      <div
                        className={`w-8 h-8 bg-[#fff3e0] rounded-full flex items-center justify-center text-sm font-bold border-2 border-slate-400 shadow-md ${h.number === 6 || h.number === 8 ? "text-red-600" : "text-slate-800"}`}
                      >
                        {h.number}
                      </div>
                    )}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: e.owner ? "0 0 10px rgba(0,0,0,0.5)" : "",
                  }}
                />
              );
            })}

            {Object.values(gameState.board.edges)
              .filter((e) => e.port)
              .map((e) => {
                ...
                const isN1Outward =
                  nx1 * e.x + ny1 * e.y > nx2 * e.x + ny2 * e.y;
                ...
                return (
                  <div
                    key={`port-${e.id}`}
                    className="absolute w-8 h-8 -ml-4 -mt-4 bg-cyan-900/80 rounded-full border-2 border-cyan-400 flex items-center justify-center shadow-lg"
                    style={{ left: portX, top: portY, zIndex: 50 }}
                  >
                    <div
                      className="absolute w-1 bg-cyan-400/50 rounded -z-10"
                      style={{
                        height: pushDistance,
                        top: "50%",
                        left: "50%",
                        transformOrigin: "top center",
                        transform: `translate(-50%, 0) rotate(${pierAngle - 90}deg)`,
                      }}
                    />
            ...
            {Object.entries(gameState.board.nodes).map(([nId, n]) => {
              const isSettlementMode =
                activeBuildMode === "SETTLEMENT" ||
                gameState.turnPhase.startsWith("SETUP_SETTLEMENT");
              const canBuildSettlement =
                isMyTurn && isSettlementMode && isValidSettlement(nId);
              const canBuildCity =
                isMyTurn &&
        ...
        <div className="h-auto pb-4 pt-4 md:h-32 bg-slate-900/95 border-t-2 border-orange-500/50 backdrop-blur-xl z-[60] flex flex-col justify-center px-4 relative shrink-0">
                ...
                .map(([key, def]) => (
                  ...
                  >
                    ...
                    {resourceAnimations[key]?.map((anim) => (
                      <div
                        key={anim.id}
                        className={`absolute -top-6 left-1/2 -translate-x-1/2 text-sm font-black ${anim.val > 0 ? "text-emerald-400" : "text-red-400"} animate-float-up pointer-events-none drop-shadow-md z-50`}
                      >
                        {anim.val > 0 ? "+" : ""}
                        {anim.val}
                      </div>
          ...
          {gameState.status === "finished" ? (
            <div className="w-full flex flex-col items-center justify-center py-2">
              <h3 className="text-xl font-black text-orange-400 tracking-widest animate-pulse mb-1">
                MISSION TERMINATED
              </h3>
              ...
              <button
                ...
                className="px-6 py-3 bg-orange-600/20 border border-orange-500/50 hover:bg-orange-600/40 rounded-xl text-orange-300 font-bold transition-all shadow-[0_0_15px_rgba(249,115,22,0.2)] flex items-center gap-2"
              >
                <Trophy size={18} />
            ...
            <div className="flex flex-col md:flex-row items-center justify-between w-full max-w-5xl mx-auto gap-4">
              <div className="w-full md:w-1/4 text-center md:text-left hidden md:block">
                ...
                <div
                  className={`text-sm md:text-lg font-black uppercase ${isMyTurn ? "text-orange-400 animate-pulse" : "text-slate-500"}`}
                >
                  {isMyTurn
                    ? getPhaseDisplayName(gameState.turnPhase)
                    : `WAITING FOR ${gameState.players[gameState.turnIndex]?.name || "Departed Settler"}`}
              ...
              <div className="flex-1 flex flex-wrap justify-center gap-2">
                {isMyTurn &&
                  ...
                  !gameState.hasRolled && (
                    <button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-500 text-white font-black px-8 py-3 rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.5)] flex items-center gap-2 text-lg animate-bounce"
                    >
                      <Dices /> ROLL DICE
                    </button>
                  )}
                    ...
                    {gameState.turnPhase === "MAIN" && (
                      <>
                        <button
                          onClick={() =>
                            setActiveBuildMode(
                              activeBuildMode === "ROAD" ? null : "ROAD",
                            )
                          }
                          className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-bold flex flex-col items-center border-2 transition-all text-xs md:text-base ${activeBuildMode === "ROAD" ? "bg-blue-600 border-blue-400 scale-105 shadow-[0_0_10px_rgba(59,130,246,0.5)]" : "bg-slate-800 border-slate-600 hover:bg-slate-700"}`}
                        >
                          <Grip size={16} /> Road
                        </button>
                        <button
                          ...
                          className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-bold flex flex-col items-center border-2 transition-all text-xs md:text-base ${activeBuildMode === "SETTLEMENT" ? "bg-orange-600 border-orange-400 scale-105 shadow-[0_0_10px_rgba(249,115,22,0.5)]" : "bg-slate-800 border-slate-600 hover:bg-slate-700"}`}
                        >
                          <Home size={16} /> Set
                        </button>
                        <button
                          onClick={() =>
                            setActiveBuildMode(
                              activeBuildMode === "CITY" ? null : "CITY",
                            )
                          }
                          className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-bold flex flex-col items-center border-2 transition-all text-xs md:text-base ${activeBuildMode === "CITY" ? "bg-indigo-600 border-indigo-400 scale-105 shadow-[0_0_10px_rgba(79,70,229,0.5)]" : "bg-slate-800 border-slate-600 hover:bg-slate-700"}`}
                        >
                ...
                {isMyTurn && gameState.turnPhase === "ROAD_BUILDING_1" && (
                  <div className="flex gap-2">
                    <div className="bg-blue-900/50 px-2 py-1 md:px-4 md:py-2 rounded border border-blue-500 text-blue-200 font-bold animate-pulse flex items-center text-xs md:text-base">
                      Place 1st Free Road
                    </div>
                    <button
                      onClick={skipRoadBuilding}
                      className="bg-slate-800 hover:bg-slate-700 px-2 py-1 md:px-3 md:py-2 rounded text-slate-300 font-bold text-xs"
                    >
                      Skip
                    </button>
                  </div>
              ...
              <div className="w-full md:w-1/4 flex justify-center md:justify-end mt-2 md:mt-0">
                {isMyTurn && gameState.turnPhase === "MAIN" && (
                  <button
                    ...
                    className="bg-red-600 hover:bg-red-500 text-white font-bold px-4 py-2 md:px-8 md:py-3 rounded-xl shadow-[0_0_15px_rgba(220,38,38,0.5)] border border-red-400 transition-colors w-full md:w-auto text-sm md:text-base"
                  >
                    END TURN
                  </button>
                )}
                  ...
                  {Object.keys(getTradeRatios(user.uid, gameState.board)).map(
                    (k) => {
                      ...
                      return (
                        <div
                          key={k}
                          className={`flex items-center gap-1 px-2 py-1 rounded-md text-xs font-black ${ratio < 4 ? "bg-cyan-900/50 text-cyan-200 border border-cyan-700 shadow-[0_0_10px_rgba(6,182,212,0.3)]" : "bg-slate-900 text-slate-400 border border-slate-700"}`}
                        >
          ...
          <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl max-w-lg w-full shadow-2xl relative overflow-y-auto max-h-[90vh] custom-scrollbar">
              ...
              <h3 className="text-2xl font-black text-white mb-6 uppercase flex items-center gap-2">
                <Scroll className="text-purple-400" /> Development Cards
              </h3>
              {gameState.turnPhase === "MAIN" && (
                <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 mb-6 flex justify-between items-center">
                  <div>
                    ...
                    <div className="flex gap-1 text-xs">
                      <span className="bg-lime-900/50 text-lime-200 px-1 rounded flex items-center">
                        <PawPrint size={10} />1
                      </span>
                      <span className="bg-yellow-900/50 text-yellow-200 px-1 rounded flex items-center">
                        <Leaf size={10} />1
                      </span>
                      <span className="bg-slate-700 text-slate-200 px-1 rounded flex items-center">
                        <Mountain size={10} />1
                      </span>
                    </div>
                  </div>
                  <button
                    ...
                    className="bg-purple-600 hover:bg-purple-500 text-white font-bold px-4 py-2 rounded-lg shadow-lg"
                  >
                    Draw
                  </button>
                </div>
              ...
              <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="bg-slate-900 p-2 rounded-lg">
                          <def.icon className="text-purple-400" size={20} />
                        </div>
                        <div>
                          ...
                          {newCount > 0 && type !== "VP" && (
                            <div className="text-[10px] text-amber-500">
                              ({newCount} bought this turn, cannot play)
                            </div>
                          )}
                        </div>
                      </div>
                      {count > 0 && (
                        <button
                          ...
                          disabled={type !== "VP" && me.hasPlayedDevCard}
                          className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:hover:bg-emerald-600 text-white font-bold px-3 py-1.5 rounded-lg text-sm whitespace-nowrap shadow-lg"
                        >
                          {type === "VP" && count > 1
                            ? `Play All (${count})`
                            : "Play"}
                ...
                {Object.values(me.devCards).reduce((a, b) => a + b, 0) +
                  Object.values(me.newDevCards).reduce((a, b) => a + b, 0) ===
                  0 && (
              ...
              <div className="mt-8 pt-4 border-t border-slate-800">
                ...
                <div className="space-y-3">
                  {gameState.players.map((p) => {
                    const playedCards = p.usedDevCards || {
                      KNIGHT: 0,
                      VP: 0,
                      ROAD: 0,
                      PLENTY: 0,
                    ...
                    if (
                      Object.values(playedCards).reduce((a, b) => a + b, 0) ===
                      0
                    )
                      return null;
                    return (
                      <div
                        key={p.id}
                        className="bg-slate-800/40 p-2 rounded-lg border border-slate-700/50"
                      >
                        <span
                          ...
                          {Object.keys(playedCards).map((type) => {
                            if (playedCards[type] > 0) {
                              ...
                              return (
                                <div
                                  key={type}
                                  className="flex items-center gap-1 bg-slate-900 px-2 py-1 rounded text-xs text-slate-300 font-bold border border-slate-700"
                                >
                                  <Icon size={12} className="text-purple-400" />{" "}
                                  {DEV_CARD_TYPES[type].name} x
                                  {playedCards[type]}
                                </div>
                              );
                            }
        ...
        {gameState.status === "finished" && !hideGameOverModal && (
          <GameOverModal
            ...
            onHide={() => setHideGameOverModal(true)}
          />
        )}
        <GameLogo />
      </div>
